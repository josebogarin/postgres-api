...
@router.post("/reset-log", summary="Limpiar el log de llamadas API")
async def reset_api_log(current: CurrentUser, db: DBSession) -> dict:
    await _require_admin(current)
    await db.execute(text("DELETE FROM api_sync_log"))
    await db.commit()
