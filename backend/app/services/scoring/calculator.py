...
class ScoringCalculator:
    ...
    async def calculate(self, torneo_id: int, engine: ScoringEngine, force_grupos: bool = False) -> dict | None:
        """
        ...
        Si force_grupos=True, recalcula también fases de grupo bloqueadas.
        """
        db = self.db

        # 1) Cargar partidos finalizados (con columnas v2)
        partidos = await self._load_partidos(db, torneo_id, force_grupos=force_grupos)
        if not partidos:
            return None

        # 2) Cargar apuestas (con columnas v2)
        apuestas = await self._load_apuestas(db, torneo_id, force_grupos=force_grupos)
        ...
        # Si force_grupos=True, también borra y recalcula fases de grupo bloqueadas.
        _bloq_clause = "TRUE" if force_grupos else "COALESCE(f.bloqueada, FALSE) = FALSE"
        await db.execute(
            text(f"""DELETE FROM puntaje_detalle
                    WHERE torneo_id = :tid
                      AND partido_id IN (
                          SELECT p.id FROM partido p
                          JOIN fase f ON f.id = p.fase_id
                          WHERE f.torneo_id = :tid AND {_bloq_clause}
                      )"""),
            {"tid": torneo_id},
        )
        ...
        try:
            await db.execute(
                text(f"""DELETE FROM puntaje_item
    ...
    async def calculate_global(self, torneo_id: int, engine) -> dict:
        ...
        # NOTA: Solo D (peor equipo) puntúa durante el torneo.
        # A/B/C/E/F/G: solo puntúan cuando hay campeón (torneo terminado).
        # Mientras no haya campeón, esos ítems devuelven 0 pts.
    ...
    async def _load_torneo_resultados(self, db, torneo_id: int) -> dict:
        ...
        # C — Goleador (admin-set). Solo puntúa al final del torneo.
        # provisional_goleador: para UI informativa (no puntúa hasta que haya campeón).
        try:
            r = await db.execute(
            ...
            _goleador_val = (row or {}).get("resultado_goleador")
            result["provisional_goleador"] = _goleador_val  # solo informativo
            if result.get("campeon_id") is not None:        # puntúa solo al final
                result["goleador"] = _goleador_val
        except Exception:
            result["provisional_goleador"] = None
        ...
        # E — Mayor goleada del torneo. Solo puntúa al final del torneo.
        # El dato (ganador/perdedor) cubre TODOS los partidos del mundial.
        if result.get("campeon_id") is not None:
            try:
                r = await db.execute(
                    text("""
                ...
                if goleada:
                    ...
                    result["goleada_ganadores_set"]   = [real_g]
                    result["goleada_perdedores_set"]  = [real_p]
            except Exception:
                pass

        # F + G — Etapa y goles de Paraguay.
        # Se calculan en cuanto Paraguay esté eliminado (no necesita esperar al campeón).
        # La acumulación cubre TODOS los partidos del mundial (no solo una fase).
        try:
            paraguay_ids = await self._get_paraguay_ids(db)
            if paraguay_ids:
    ...
    async def _load_partidos(self, db, torneo_id: int, force_grupos: bool = False) -> dict[int, dict]:
        _bloq = "TRUE" if force_grupos else "COALESCE(f.bloqueada, FALSE) = FALSE"
        r = await db.execute(
            text(f"""
    ...
    async def _load_apuestas(self, db, torneo_id: int, force_grupos: bool = False) -> list[dict]:
        _bloq = "TRUE" if force_grupos else "COALESCE(f.bloqueada, FALSE) = FALSE"
    ...
    def _check_teams_match(
        ...
        if tipo == "grupo":
            return True
        # Todas las fases KO: return True.
        # Copa del Mundo 2026 tiene bracket fijo de FIFA. Los apostadores vieron los
        # equipos reales al apostar cada fase KO (R32 importado via Excel; R16+ via UI).
        # La simulacion bracket desde standings de grupo no reproduce el bracket real.
        return True
