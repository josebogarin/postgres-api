...
calc AS (
    SELECT
        ...
        CASE WHEN p.minuto_primer_gol IS NOT NULL
              AND ABS(COALESCE(a.pred_minuto_gol,0) - p.minuto_primer_gol) = mp.min_diff
    ...
    LEFT JOIN min_por_partido mp ON mp.partido_id = p.id
...
SELECT apostador_id, nombre_apostador, numero_fifa,
       min_real, pred_min, distancia, min_diff,
       N_esp, N_bd,
