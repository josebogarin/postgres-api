@echo off
echo Consultando eventos Card en BD...
docker exec core-postgres psql -U app_user -d becbuc -c ^
"SELECT p.numero_fifa, ev->>'detail' AS tipo, ^
 ...
 LIMIT 50;" > "%~dp0resultado_tarjetas.txt" 2>&1

echo Resultado guardado en resultado_tarjetas.txt
type "%~dp0resultado_tarjetas.txt"
pause
