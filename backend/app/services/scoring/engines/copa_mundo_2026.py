...
FASES: dict[str, FaseConfig] = {
    "grupo": FaseConfig(pts_resultado=4, pts_marcador_exacto=8, pts_penales_tanda_por_equipo=0, pts_equipo_clasifica=0),  # P grupos: manejado por apostador_clasificados (no por partido)
    ...
    "tercer_puesto": FaseConfig(pts_resultado=14, pts_marcador_exacto=28, pts_penales_tanda_por_equipo=2, pts_equipo_clasifica=10),
...
class CopasMundoScoringEngine:
    ...
    def score_partido(self, apuesta, partido, fase_tipo, es_paraguay=False, ko_teams_match=True):
        ...
        # Regla: victoria predicha → clasificado se infiere del marcador (pred_equipo_clasifica ignorado).
        #        empate predicho  → va a prórroga + penales → pred_equipo_clasifica define quién pasa.
        equipo_clasifica_real = partido.get("equipo_clasificado_id")
        if equipo_clasifica_real is not None and cfg.pts_equipo_clasifica > 0:
            ...
            if pl is not None and pv is not None:
                if pl > pv:
                    # Victoria local predicha → clasifica local directamente
                    pred_clasifica = equipo_local_id
                elif pv > pl:
                    # Victoria visitante predicha → clasifica visitante directamente
                    pred_clasifica = equipo_visitante_id
                else:
                    # Empate predicho → 120 min → penales → pred_equipo_clasifica es la clave
                    pred_clasifica = apuesta.get("pred_equipo_clasifica")
                    if pred_clasifica is None:
                        # Fallback: inferir de tanda pronosticada si no completó el campo
                        pt_l = apuesta.get("pred_penales_local_tanda")
                        pt_v = apuesta.get("pred_penales_visitante_tanda")
                        if pt_l is not None and pt_v is not None:
                            if pt_l > pt_v:
                                pred_clasifica = equipo_local_id
                            elif pt_v > pt_l:
                                pred_clasifica = equipo_visitante_id

            if pred_clasifica is not None and pred_clasifica == equipo_clasifica_real:
                # Item P (equipo que clasifica) NO se duplica para Paraguay (decision organizacion).
                score.pts_equipo = cfg.pts_equipo_clasifica
