...
async def _log_warn_rich(
    db: AsyncSession,
    contexto: str,
    params_data: dict | None = None,
    numero_fifa: int | None = None,
    fase_nm: str | None = None,
    local_nm: str | None = None,
    visitante_nm: str | None = None,
) -> None:
    """Inserta advertencia en api_sync_log con params JSONB + columnas de partido."""
    try:
        await db.execute(text("SAVEPOINT _logwarnr_sp"))
        try:
            await db.execute(
                text("""
                    INSERT INTO api_sync_log
                        (endpoint, params, status_code, response_ms,
                         quota_remaining, error_msg, payload_size, origen, contexto,
                         numero_fifa, fase_nm, local_nm, visitante_nm)
                    VALUES ('sync', :params, NULL, NULL, NULL, NULL, NULL, 'sync', :ctx,
                            :num_fifa, :fase_nm, :local_nm, :visitante_nm)
                """),
                {
                    "ctx": contexto,
                    "params": json.dumps(params_data or {}),
                    "num_fifa": numero_fifa,
                    "fase_nm": fase_nm,
                    "local_nm": local_nm,
                    "visitante_nm": visitante_nm,
                },
            )
            await db.execute(text("RELEASE SAVEPOINT _logwarnr_sp"))
        except Exception:
            try:
                await db.execute(text("ROLLBACK TO SAVEPOINT _logwarnr_sp"))
            except Exception:
                pass
    except Exception:
        pass
...
async def _espn_verify_and_patch(
    ...
    if espn_amar > (current.get("amarillas") or 0):
        corrections["amarillas"]      = espn_amar
    if espn_roja > (current.get("rojas") or 0):
...
async def sync_torneo(
            ...
            text("""
                ...
                WHERE p.torneo_id = :tid
                  ...
                  AND NOT EXISTS (
                      ...
                      WHERE id = p.equipo_local_id
                        AND (nombre = 'TBD' OR nombre = 'Por Definir')
                  )
                  AND NOT EXISTS (
                      SELECT 1 FROM equipo
                      WHERE id = p.equipo_visitante_id
                        AND (nombre = 'TBD' OR nombre = 'Por Definir')
                  )
            """),
            {"tid": torneo_id},
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        for fix_id, db_p in db_partidos.items():
            ...
            if fecha:
                try:
                    ...
                    in_window = 0.0 <= elapsed_min <= 240.0  # cubre reg(90)+AET(30)+penales(30)+margen(90)
                except Exception:
                    pass
        ...
        for fix_id, db_p, is_live, resp, t0, err in fetch_results:
            ...
            try:
                ...
                prev_estado = db_p.get("estado") or ""
                # Log inicio de partido
                _live_statuses = {"1H", "HT", "2H", "ET", "BT", "P", "LIVE"}
                if fix_status in _live_statuses and prev_estado not in ("en_juego",):
                    await _log_warn_rich(
                        db, f"\u26bd Inicio: {match_name}",
                        numero_fifa=db_p.get("numero_fifa"),
                        local_nm=db_p.get("local_nombre"), visitante_nm=db_p.get("visit_nombre"),
                    )
                    await db.commit()
                # Log fin de partido con resultado
                elif fix_status in STATUS_FINAL and prev_estado != "finalizado":
                    _gl = fix_full.get("goals", {}).get("home")
                    _gv = fix_full.get("goals", {}).get("away")
                    _pen_str = ""
                    if fix_status == "PEN":
                        _pl = fix_full.get("score", {}).get("penalty", {}).get("home")
                        _pv = fix_full.get("score", {}).get("penalty", {}).get("away")
                        if _pl is not None and _pv is not None:
                            _pen_str = f" (pen {_pl}\u2013{_pv})"
                    _res_str = f"{_gl}\u2013{_gv}{_pen_str}" if _gl is not None else "?"
                    await _log_warn_rich(
                        db, f"\u2705 Final: {match_name} \u00b7 {_res_str}",
                        numero_fifa=db_p.get("numero_fifa"),
                        local_nm=db_p.get("local_nombre"), visitante_nm=db_p.get("visit_nombre"),
                    )
                    await db.commit()
    ...
    if espn_corrections:
        for _ec in espn_corrections:
            _ec_pid = _ec["partido_id"]
            try:
                _pr = await db.execute(
                    text("""
                        SELECT p.numero_fifa,
                               COALESCE(f.nombre, f.tipo, '?') AS fase_nm,
                               COALESCE(el.nombre_es, el.nombre, '?') AS local_nm,
                               COALESCE(ev.nombre_es, ev.nombre, '?') AS visit_nm,
                               p.eventos_api
                        FROM partido p
                ...
                _prow = dict(_pr.mappings().first() or {})
                _num = _prow.get("numero_fifa") or _ec_pid
                _fase = _prow.get("fase_nm", "?")
                _loc  = _prow.get("local_nm", "?")
                _vis  = _prow.get("visit_nm", "?")
                _evts = _prow.get("eventos_api") or []
                _ctx = f"ESPN ✓ P{_num} {_fase}: {_loc} - {_vis}"
                await _log_warn_rich(db, _ctx, {
                    "num_fifa": _num,
                    "fase": _fase,
                    "local": _loc,
                    "visitante": _vis,
                    "correcciones": _ec.get("corr", {}),
                    "eventos": _evts[:15] if isinstance(_evts, list) else [],
                },
                numero_fifa=_num if isinstance(_num, int) else None,
                fase_nm=_fase,
                local_nm=_loc,
                visitante_nm=_vis,
                )
            except Exception as _elog_e:
                await _log_warn(db, f"ESPN corrección partido {_ec_pid}: {_ec.get('corr', {})}")
    if ss_corrections:
...
async def _process_live_pending_updates(
    ...
    for pid, db_p, estado_target, minuto_target in to_update:
        try:
            # Solo actualizar estado en partido_stats_fuentes — los resultados
            # los actualiza únicamente el admin manualmente via sync/ESPN.
            await db.execute(text("SAVEPOINT _lpend_sp"))
            await db.execute(
                text("""
                    UPDATE partido_stats_fuentes
                    SET estado          = :estado,
                        ultimo_minuto   = COALESCE(:minuto, ultimo_minuto),
                        synced_at       = NOW()
                    WHERE partido_id = :pid
                """),
                {"estado": estado_target, "minuto": minuto_target, "pid": pid},
            )
            await db.execute(text("RELEASE SAVEPOINT _lpend_sp"))
            await db.commit()
            actualizados += 1

        except Exception as e:
            try:
                await db.execute(text("ROLLBACK TO SAVEPOINT _lpend_sp"))
            except Exception:
                pass
            await db.rollback()
            errores += 1
...
async def _update_partido_full(
    ...
    for ev in events_sorted:
        ...
        if ev_type == "Card":
            # Ignorar tarjetas al banco/cuerpo técnico.
            # API-Football devuelve coach/staff cards con player.id = null.
            # El reglamento BECBUC solo cuenta tarjetas a jugadores de campo.
            _ev_player_id = (ev.get("player") or {}).get("id")
            if _ev_player_id is None:
                pass  # coach/staff card — no contar
        ...
        # Penal cobrado durante el juego (convertido o fallado) — ítem M.
        # EXCLUIR tanda de penales (ítem O): los kicks de tanda NO deben sumarse aquí.
        #
        # Discriminadores para excluir kicks de tanda:
        #   1. comments = "Penalty Shootout" (API-Football v3 lo incluye en muchos casos)
        #   2. Si el partido fue a tanda (pen_home not None): la tanda empieza al
        #      terminar el tiempo extra (elapsed=120). API-Football reporta los kicks
        #      de tanda CON elapsed=120 (mismo valor que el fin del tiempo extra),
        #      por eso usamos elapsed < 120 (estricto) cuando hubo tanda.
        #      Si NO hubo tanda: usamos elapsed <= 120 para no perder penales
        #      legítimos en el minuto exacto 120 de prórroga.
        _ev_elapsed  = ev.get("time", {}).get("elapsed")
        _ev_comments = (ev.get("comments") or "").lower()
        _is_shootout_kick = "shootout" in _ev_comments
        # Umbral: 119 cuando hubo tanda (excluye elapsed=120 de tanda); 120 si no.
        _elapsed_max  = 119 if pen_home is not None else 120
        _is_in_play   = (
            not _is_shootout_kick and
            _ev_elapsed is not None and
            _ev_elapsed <= _elapsed_max
        )
        if ev_type == "Goal" and ev_detail == "Penalty" and _is_in_play:
            penales_partido_total += 1
        elif ev_type == "Miss" and ev_detail in ("Missed Penalty", "Penalty Missed") and _is_in_play:
            penales_partido_total += 1
        elif ev_type == "Goal" and ev_detail in ("Missed Penalty", "Penalty Missed") and _is_in_play:
    ...
    await db.execute(
        text("""
            UPDATE partido SET
                ...
                eventos_api            = COALESCE(CAST(:eventos_json AS jsonb), eventos_api)
            WHERE id = :pid
        """),
        {
            "gl":      goals_home,
            ...
            "vis_ro":  _vis_rojas,
            "pid":     partido_id,
            "eventos_json": json.dumps(events_sorted) if events else None,
        },
    )
    await db.commit()
