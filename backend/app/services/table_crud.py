...
def _coerce_value(col, val: Any) -> Any:
    ...
    if any(t in type_name for t in ("TIMESTAMP", "DATETIME")) and isinstance(val, str):
        ...
        if not v:
            return None
        # Intentar fromisoformat (Python 3.11+ maneja timezone, versiones anteriores no)
        try:
            return datetime.fromisoformat(v)
        except ValueError:
            pass
        # Fallback: eliminar offset de timezone (+00:00, -06:00, Z) y parsear naive
        v_clean = _re.sub(r"(Z|[+\-]\d{2}:?\d{2})$", "", v).strip()
        v_clean = v_clean.replace("T", " ")
        # Truncar milisegundos si los hay
        v_clean = v_clean[:19]
