# -*- coding: utf-8 -*-
"""
importar_cuartos_excel.py
Importa las apuestas (predicciones) de CUARTOS DE FINAL (P097-P100) por apostador
desde el Excel consolidado "PRONOSTICOS CONSOLIDADOS 4TOS.xlsx" (hoja '4tos').

Estructura Excel (hoja '4tos', 1-based):
  col 2  = ID PARTIDO (P097..P100)   col 7  = FASE ('40- CUARTOS')
...
Uso:
  backend\.venv\Scripts\python.exe importar_cuartos_excel.py            <- DRY RUN
  backend\.venv\Scripts\python.exe importar_cuartos_excel.py --import   <- ESCRIBE en BD
"""
import sys, os, glob
...
for f in os.listdir(BASE):
    fu = f.upper()
    if ('4TOS' in fu or 'CUARTOS' in fu) and f.endswith('.xlsx'):
        EXCEL_FILE = os.path.join(BASE, f); break
if not EXCEL_FILE:
    sys.exit(f"ERROR: no se encontro Excel *4TOS*.xlsx en {BASE}")
...
EQUIPO_ALIAS = {
    'FRANCIA':'France','MARRUECOS':'Morocco','ESPAÑA':'Spain','ESPANA':'Spain',
    'BELGICA':'Belgium','NORUEGA':'Norway','INGLATERRA':'England',
    'ARGENTINA':'Argentina','SUIZA':'Switzerland',
}
...
cur_bec.execute("""
    ...
    WHERE f.torneo_id = %s AND p.numero_fifa BETWEEN 97 AND 100
    ORDER BY p.numero_fifa
""", (TORNEO_ID,))
partidos_c = {f"P{r['numero_fifa']:03d}": dict(r) for r in cur_bec.fetchall()}
print(f"\nPartidos CUARTOS en BD ({len(partidos_c)}):")
...
for r in range(2, ws.max_row + 1):
    if str(ws.cell(r, 7).value) != '40- CUARTOS':
        continue
...
print(f"\nPronosticos resueltos: {len(preds)}  (esperado 44 x 4 = 176)")
...
if not DO_IMPORT:
    ...
    print("\nPara ESCRIBIR en BD: importar_cuartos_excel.py --import")
    sys.exit(0)

# ── Import ────────────────────────────────────────────────────────────────────
print(f"\nIMPORTANDO {len(preds)} apuestas de cuartos...")
up, err = 0, 0
for pred in preds:
    try:
        cur_bec.execute("""
...
""")
v = cur_bec.fetchone()
print(f"Verificacion BD cuartos: {v['total']} apuestas ({v['aps']} apostadores x {v['parts']} partidos)")
print("\nListo. Luego recalcular: POST /calcular-puntajes/2 (cuando terminen los cuartos).")
