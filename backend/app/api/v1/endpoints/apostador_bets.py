...
# ── Online tracking (in-memory, se resetea al reiniciar el servidor) ───────────
import time as _time

_ONLINE_TTL = 120  # segundos para considerar "activo"
_LIVE_TTL   =  90  # segundos para presencia live-playoffs (loadLive corre cada 30s)

# {user_id: (timestamp_float, source_str)}  source: "web" | "movil"
_online_users: dict[int, tuple[float, str]] = {}

# {apostador_id: (timestamp_float, alias_str, torneo_id)}  — sin auth, desde live-playoffs
...
async def heartbeat(source: str = "web", viewas_id: int = None, current_user: OptionalCurrentUser = None):
    """source: 'web' | 'movil' | 'playoffs'. TTL = _ONLINE_TTL segundos. Auth opcional.
    viewas_id: si el llamador es admin, registra ese ID en lugar del propio (usado por live-playoffs)."""
    if current_user is not None:
        uid = current_user.id
        _online_users[uid] = (_time.monotonic(), source)
        # Persistir actividad diaria en app_db
        _alias = ""
        try:
            _alias = str(getattr(current_user, "username", "") or "")
        except Exception:
            pass
        try:
            # CREATE TABLE en transacción separada (DDL idempotente)
            async with _app_engine.begin() as _aconn:
                await _aconn.execute(text("""
                    CREATE TABLE IF NOT EXISTS usuario_actividad_dia (
                        id SERIAL PRIMARY KEY,
                        user_id BIGINT NOT NULL,
                        alias VARCHAR(200),
                        fecha DATE DEFAULT CURRENT_DATE,
                        primera_conexion TIMESTAMP DEFAULT NOW(),
            ...
            print(f"[heartbeat] CREATE TABLE warn: {_e}")
        try:
            async with _app_engine.begin() as _aconn:
                # UPDATE primero; si no existe la fila de hoy, INSERT (evita ON CONFLICT que requiere constraint)
                r = await _aconn.execute(text("""
                    UPDATE usuario_actividad_dia
                    SET ultima_conexion = NOW(), dispositivo = :dispositivo,
                        alias = CASE WHEN alias = '' OR alias IS NULL THEN :alias ELSE alias END
                    WHERE user_id = :uid AND fecha = CURRENT_DATE
                """), {"uid": uid, "alias": _alias, "dispositivo": source})
                if r.rowcount == 0:
                    await _aconn.execute(text("""
                        INSERT INTO usuario_actividad_dia
                            (user_id, alias, fecha, primera_conexion, ultima_conexion, dispositivo)
                        VALUES (:uid, :alias, CURRENT_DATE, NOW(), NOW(), :dispositivo)
                    """), {"uid": uid, "alias": _alias, "dispositivo": source})
        except Exception as _e:
            print(f"[heartbeat] INSERT error uid={uid} alias={_alias!r}: {_e}")
    return {"ok": True}


@router.get("/usuarios-hoy", summary="Usuarios que se conectaron hoy")
async def usuarios_hoy() -> list[dict]:
    """Retorna usuarios que hicieron heartbeat hoy, con alias y dispositivo."""
    try:
        async with _app_engine.connect() as _aconn:
            r = await _aconn.execute(text("""
                SELECT user_id, COALESCE(alias, '') AS alias,
                       dispositivo, primera_conexion, ultima_conexion
                FROM usuario_actividad_dia
                WHERE fecha = CURRENT_DATE
                ORDER BY ultima_conexion DESC
            """))
            rows = r.mappings().all()
            db_result = [{"user_id": row["user_id"], "alias": row["alias"],
                          "dispositivo": row["dispositivo"],
                          "primera_conexion": row["primera_conexion"].isoformat() if row["primera_conexion"] else None,
                          "ultima_conexion": row["ultima_conexion"].isoformat() if row["ultima_conexion"] else None}
                         for row in rows]
            # Completar alias vacíos y agregar usuarios online no persistidos aún
            db_uids = {row["user_id"] for row in db_result}
            now_m = _time.monotonic()
            online_uids = [uid for uid, (ts, _src) in _online_users.items()
                           if (now_m - ts) < _ONLINE_TTL and uid not in db_uids]
            if online_uids or any(not row["alias"] for row in db_result):
                ids_needed = list({uid for uid in online_uids}
                                  | {row["user_id"] for row in db_result if not row["alias"]})
                if ids_needed:
                    ids_sql = ",".join(str(i) for i in ids_needed)
                    ar = await _aconn.execute(text(
                        f"SELECT id, username FROM users WHERE id IN ({ids_sql})"
                    ))
                    alias_map = {row2["id"]: row2["username"] for row2 in ar.mappings()}
                    for row in db_result:
                        if not row["alias"]:
                            row["alias"] = alias_map.get(row["user_id"], f"U{row['user_id']}")
                    for uid in online_uids:
                        src = _online_users.get(uid, (0, "web"))[1]
                        db_result.append({
                            "user_id": uid,
                            "alias": alias_map.get(uid, f"U{uid}"),
                            "dispositivo": src,
                            "primera_conexion": None,
                            "ultima_conexion": None,
                        })
            return db_result
    except Exception as _e:
        print(f"[usuarios-hoy] error: {_e}")
        return []


@router.post("/live-presencia", summary="Registra presencia de apostador en página live (sin auth)")
async def live_presencia_post(apostador_id: int, torneo_id: int = 2, alias: str = ""):
    """Llamado desde becbuc-live-playoffs.html en cada loadLive (cada 30s).
    No requiere auth — apostador_id y alias provienen del selector de la página.
    Registra al apostador en _online_users y usuario_actividad_dia para que aparezca en Conectados hoy."""
    if apostador_id > 0:
        _alias = alias or f"U{apostador_id}"
        _live_presencia[apostador_id] = (_time.monotonic(), _alias, torneo_id)
        # También registrar en _online_users para que aparezca en usuarios-online
        _online_users[apostador_id] = (_time.monotonic(), "playoffs")
        # Persistir en usuario_actividad_dia para "Conectados hoy"
        try:
            async with _app_engine.begin() as _aconn:
                await _aconn.execute(text("""
                    CREATE TABLE IF NOT EXISTS usuario_actividad_dia (
                ...
                """))
                # UPDATE primero; si no existe la fila, INSERT
                r = await _aconn.execute(text("""
                    UPDATE usuario_actividad_dia
                    SET ultima_conexion = NOW(), dispositivo = 'playoffs',
                        alias = CASE WHEN alias = '' OR alias IS NULL THEN :alias ELSE alias END
                    WHERE user_id = :uid AND fecha = CURRENT_DATE
                """), {"uid": apostador_id, "alias": _alias})
                if r.rowcount == 0:
                    await _aconn.execute(text("""
                        INSERT INTO usuario_actividad_dia
                            (user_id, alias, fecha, primera_conexion, ultima_conexion, dispositivo)
                        VALUES (:uid, :alias, CURRENT_DATE, NOW(), NOW(), 'playoffs')
                    """), {"uid": apostador_id, "alias": _alias})
        except Exception as _e:
            print(f"[live-presencia] DB error uid={apostador_id}: {_e}")
    return {"ok": True}


@router.get("/usuarios-online", summary="Lista de usuarios actualmente conectados por fuente")
async def usuarios_online(db: DBSession) -> dict:
    """
    Retorna usuarios conectados en los últimos _ONLINE_TTL segundos, agrupados por fuente.
    Incluye alias (apostador/username) desde app_db.
    """
    now = _time.monotonic()
    activos = {uid: src for uid, (ts, src) in _online_users.items()
               if (now - ts) < _ONLINE_TTL}
    if not activos:
        return {"total": 0, "web": [], "movil": [], "playoffs": []}
    ids = list(activos.keys())
    try:
        async with _app_engine.connect() as conn:
            r = await conn.execute(
                text("SELECT id, username AS alias FROM users WHERE id = ANY(:ids)"),
                {"ids": ids},
            )
            alias_map = {row["id"]: row["alias"] for row in r.mappings()}
    except Exception:
        alias_map = {}
    result: dict[str, list] = {"web": [], "movil": [], "playoffs": []}
    for uid, src in activos.items():
        key = src if src in result else "web"
        result[key].append(alias_map.get(uid, f"U{uid}"))

    # Presencia live-playoffs (sin auth, alias directo)
    now2 = _time.monotonic()
    live_activos = {uid: alias for uid, (ts, alias, _tid) in _live_presencia.items()
                    if (now2 - ts) < _LIVE_TTL}
    for uid, alias in live_activos.items():
        lbl = alias_map.get(uid) or alias
        if lbl not in result["playoffs"]:
            result["playoffs"].append(lbl)
    ...
    r = await db.execute(
        text("""
            SELECT
                ...
                COALESCE(a.pred_equipo_clasifica,        NULL) AS pred_equipo_clasifica,
...
async def live_guardar_apuestas(torneo_id: int, body: LiveGuardarIn, db: DBSession) -> dict:
    ...
    return {"ok": guardadas > 0, "guardadas": guardadas,
            "total": len(body.apuestas), "resultados": resultados, "apostador": username, "nombre": nombre_full}
    ...
    for r in rp.mappings():
        ...
        out.append({
            ...
            "provisional": (r["local_id"] is None or r["visit_id"] is None
                            or (r["local_nombre_es"] or r["local_nombre"]) == "TBD"
                            or (r["visit_nombre_es"] or r["visit_nombre"]) == "TBD"),
            "local": ({"id": r["local_id"],
                       "nombre": r["local_nombre_es"] or r["local_nombre"],
                       "logo_url": r["local_logo"],
                       "iso": r["local_iso"]}
                      if r["local_id"] and (r["local_nombre_es"] or r["local_nombre"]) != "TBD"
                      else None),
            "visitante": ({"id": r["visit_id"],
                           "nombre": r["visit_nombre_es"] or r["visit_nombre"],
                           "logo_url": r["visit_logo"],
                           "iso": r["visit_iso"]}
                          if r["visit_id"] and (r["visit_nombre_es"] or r["visit_nombre"]) != "TBD"
                          else None),
            "sede": r["sede"],
            "ciudad": r["ciudad"],
        })
...
# ─────────────────────────────────────────────────────────────────────────────
@router.get("/exportar-pronosticos/{torneo_id}",
...
@router.get("/ultimo-calculado/{torneo_id}", summary="Último partido con puntaje calculado")
async def ultimo_calculado(torneo_id: int, db: DBSession) -> dict:
    """Devuelve el partido más reciente que tiene entradas en puntaje_detalle,
    con sus ítems (goles, fase, fecha) y cuántos apostadores fueron calculados."""
    try:
        row = await db.execute(text("""
            SELECT
                ...
                el.nombre AS equipo_local,
                ev.nombre AS equipo_visitante,
                COALESCE(el.codigo_iso, '') AS iso_local,
                COALESCE(ev.codigo_iso, '') AS iso_visitante,
                f.nombre   AS fase_nombre,
                f.tipo     AS fase_tipo,
            ...
            JOIN equipo el ON el.id = p.equipo_local_id
            JOIN equipo ev ON ev.id = p.equipo_visitante_id
            JOIN puntaje_detalle pd ON pd.partido_id = p.id
            WHERE f.torneo_id = :tid
            GROUP BY p.id, p.fecha, p.goles_local, p.goles_visitante,
                     p.penales_local, p.penales_visitante, p.estado,
                     el.nombre, ev.nombre, el.codigo_iso, ev.codigo_iso,
                     f.nombre, f.tipo, p.numero_fifa
            ORDER BY p.fecha DESC NULLS LAST, p.numero_fifa DESC
            LIMIT 1
        """), {"tid": torneo_id})
        r = row.mappings().first()
        if not r:
            return {"ok": False, "partido": None}
        d = dict(r)
        if d.get("fecha"):
            d["fecha"] = d["fecha"].strftime("%Y-%m-%dT%H:%M:%SZ")
        return {"ok": True, "partido": d}
    except Exception as e:
        return {"ok": False, "error": str(e), "partido": None}
...
async def ranking(torneo_id: int, db: DBSession) -> list[dict]:
    ...
    # ── pts_equipo por fase KO (desglose P por fase en árbol) ──────────────────
    clasifica_ko_by_uid: dict[int, dict] = {}
    try:
        _sql_ko_p = text(
            "SELECT pd.apostador_id, f.tipo AS fase_tipo, "
    ...
    for row in rows:
        ...
        fases_list = list(fases_by_uid.get(uid, []))
        # Sumar pts_grupos_p (ítem P-clasificados) a la fase de grupos para que
        # la columna "Grupos" en el dashboard muestre el total correcto
        if pts_grupos_p > 0:
            for _f in fases_list:
                if (_f.get("tipo") or "").startswith("grupo"):
                    _f["pts"] = (_f.get("pts") or 0) + pts_grupos_p
                    break
            else:
                fases_list.append({"tipo": "grupo", "nombre": "Fase de grupos", "pts": pts_grupos_p})
        row["fases"]                 = fases_list
        ...
        row["globales_detalle"]      = glob_detalle_map.get(uid, {"a":0,"b":0,"c":0,"d":0,"e":0,"f":0,"g":0})
        row["clasifica_ko_fases"]    = clasifica_ko_by_uid.get(uid, {})
...
async def monitor_dashboard(torneo_id: int, current: CurrentUser, db: DBSession) -> dict:
    ...
    try:
        ...
        # Solo la primera fase abierta (la que toca ahora), no todas las futuras
        r_pend = await db.execute(
            text("""
                SELECT f.id AS fase_id, f.tipo AS fase_tipo, f.nombre AS fase_nombre,
        ...
        # Apuestas realizadas solo en esa primera fase abierta
        if fases_abiertas_info:
            _fase_id_act = fases_abiertas_info[0]["fase_id"]
            r_apost = await db.execute(
                ...
                """),
                {"tid": torneo_id, "fid": _fase_id_act},
            )
    ...
    # ── Top 5 ranking (usa puntaje_detalle — fuente única de verdad) ──────────
    try:
        # Puntos de partidos desde puntaje_detalle
        _PD_SUM = ("COALESCE(pd.pts_resultado,0) + COALESCE(pd.pts_marcador,0) + "
                   "COALESCE(pd.pts_amarillas,0) + COALESCE(pd.pts_rojas,0) + "
        ...
        r_rk = await db.execute(
            text(f"""
                SELECT pd.apostador_id,
                       COALESCE(SUM({_PD_SUM}), 0)::int AS pts_partidos,
                       SUM(CASE WHEN COALESCE(pd.pts_marcador,0) > 0 THEN 1 ELSE 0 END)::int AS plenos,
                       SUM(CASE WHEN COALESCE(pd.pts_resultado,0) > 0 AND COALESCE(pd.pts_marcador,0) = 0 THEN 1 ELSE 0 END)::int AS aciertos
                FROM puntaje_detalle pd
                WHERE pd.torneo_id = :tid
                GROUP BY pd.apostador_id
            """),
        ...
        r_glob = await db.execute(
            text("SELECT apostador_id, pts_total AS pts_globales FROM puntaje_global WHERE torneo_id = :tid"),
            {"tid": torneo_id},
        )
        glob_map = {r["apostador_id"]: r["pts_globales"] for r in r_glob.mappings()}
        # P grupos (apostador_clasificados)
        r_clas = await db.execute(
            text("SELECT apostador_id, COALESCE(pts_obtenidos, 0) AS pts_grupos_p FROM apostador_clasificados WHERE torneo_id = :tid AND fase_tipo = 'grupo'"),
            {"tid": torneo_id},
        )
        clas_map = {r["apostador_id"]: int(r["pts_grupos_p"] or 0) for r in r_clas.mappings()}
        for row in top_rows:
            ...
            row["pts_grupos_p"] = clas_map.get(row["apostador_id"], 0) or 0
            row["pts_total"]    = row["pts_partidos"] + row["pts_globales"] + row["pts_grupos_p"]
        # Ordenar y tomar top 5 en Python (ya que la query no tiene ORDER/LIMIT)
...
async def partidos_hoy(torneo_id: int, db: DBSession, fecha: str = None) -> list[dict]:
    ...
    """
    # Paraguay UTC-3 (zona horaria del dispositivo de los apostadores)
    # fecha param: YYYY-MM-DD en hora Paraguay (UTC-3); sin param = hoy en PY
    _fecha_clause = (
        "DATE(p.fecha - INTERVAL '3 hours') = CAST(:target_py AS date)"
        if fecha else
        "DATE(CURRENT_TIMESTAMP - INTERVAL '3 hours') = DATE(p.fecha - INTERVAL '3 hours')"
    )
    _params: dict = {"tid": torneo_id}
    if fecha:
        _params["target_py"] = fecha
    ...
    rp = await db.execute(
        text(f"""
            ...
            LEFT JOIN equipo ev ON ev.id = p.equipo_visitante_id
            WHERE p.torneo_id = :tid
              AND p.estado NOT IN ('aplazado', 'cancelado')
              AND {{_fecha_clause}}
            ORDER BY p.fecha ASC
        """.format(_fecha_clause=_fecha_clause)),
        _params,
    )
    result = []
    for r in rp.mappings():
...
@router.get("/admin/globales-info/{torneo_id}", summary="Datos reales para ítems globales E/F/G (sin filtro de fase bloqueada)")
async def admin_globales_info(torneo_id: int, current: CurrentAdmin, db: DBSession) -> dict:
    """
    Devuelve:
      - mayor_goleada: partido con max diferencia (ALL fases, no solo desbloqueadas)
      - goles_paraguay: total goles marcados por Paraguay en el torneo
      - etapa_paraguay: fase más avanzada donde Paraguay jugó (y finalizó)
      - goleador_registrado: resultado_goleador en tabla torneo (admin-set)
      - campeon_registrado: nombre del campeón (si la final ya se jugó)
    """
    # ── Mayor goleada (E) — todos los partidos finalizados ──────────────────
    goleada_row = (await db.execute(text("""
        SELECT el.nombre AS local, ev.nombre AS visitante,
               p.goles_local AS gl, p.goles_visitante AS gv,
               f.nombre AS fase, f.tipo AS tipo,
               ABS(p.goles_local - p.goles_visitante) AS diff
        FROM partido p
        JOIN equipo el ON el.id = p.equipo_local_id
        JOIN equipo ev ON ev.id = p.equipo_visitante_id
        JOIN fase f    ON f.id  = p.fase_id
        ...
        ORDER BY diff DESC, (p.goles_local + p.goles_visitante) DESC
        LIMIT 1
    ...
    if goleada_row:
        mayor_goleada = {
            "local": goleada_row["local"],
            "visitante": goleada_row["visitante"],
            "gl": goleada_row["gl"],
            "gv": goleada_row["gv"],
            "diff": goleada_row["diff"],
            "fase": goleada_row["fase"],
        }

    # ── Paraguay (F + G) — todos los partidos finalizados ───────────────────
    FASE_ORDER = ["grupo", "ronda32", "ronda16", "cuartos", "semis", "tercer_puesto", "tercero", "final"]
    py_rows = (await db.execute(text("""
        SELECT f.tipo AS fase_tipo,
               CASE WHEN el.nombre ILIKE '%paraguay%' THEN p.goles_local
    ...
    """), {"tid": torneo_id})).mappings().all()

    goles_paraguay = sum((r["goles_py"] or 0) for r in py_rows)
    etapa_paraguay = None
    if py_rows:
        max_idx = max((FASE_ORDER.index(r["fase_tipo"]) if r["fase_tipo"] in FASE_ORDER else -1) for r in py_rows)
        etapa_paraguay = FASE_ORDER[max_idx] if max_idx >= 0 else None

    FASE_LABELS = {
        "grupo": "Fase de Grupos", "ronda32": "Ronda de 32", "ronda16": "Ronda de 16",
        "cuartos": "Cuartos de Final", "semis": "Semifinales",
        "tercer_puesto": "3er Puesto", "tercero": "3er Puesto", "final": "Final",
    }

    # ── Goleador y campeón (admin-set / auto) ────────────────────────────────
    torneo_row = (await db.execute(text("""
        SELECT t.resultado_goleador,
               eg.nombre AS campeon_nombre
        FROM torneo t
        LEFT JOIN equipo eg ON eg.id = (
            SELECT p2.equipo_clasificado_id
            FROM partido p2 JOIN fase f2 ON f2.id = p2.fase_id
            WHERE p2.torneo_id = t.id AND f2.tipo = 'final' AND p2.estado = 'finalizado'
            LIMIT 1
        )
        WHERE t.id = :tid
    """), {"tid": torneo_id})).mappings().first()

    goleador_registrado = (torneo_row or {}).get("resultado_goleador") or None
    campeon_nombre = (torneo_row or {}).get("campeon_nombre") or None

    return {
        "mayor_goleada": mayor_goleada,
        "goles_paraguay": goles_paraguay,
        "etapa_paraguay": etapa_paraguay,
        "etapa_paraguay_label": FASE_LABELS.get(etapa_paraguay or "", etapa_paraguay or ""),
        "goleador_registrado": goleador_registrado,
        "campeon_nombre": campeon_nombre,
    }


@router.get("/torneo-cerrado/{torneo_id}", summary="Estado de cierre del torneo (para banner del live)")
async def torneo_cerrado(torneo_id: int, db: DBSession) -> dict:
    """Devuelve {cerrado: bool}. TRUE cuando cerrar_torneo_final marco el torneo como cerrado."""
    try:
        r = await db.execute(
            text("SELECT COALESCE(cerrado, FALSE) AS cerrado FROM torneo WHERE id = :tid"),
            {"tid": torneo_id},
        )
        row = r.mappings().first() or {}
        return {"cerrado": bool(row.get("cerrado"))}
    except Exception:
        try:
            await db.rollback()
        except Exception:
...
async def hay_partido_activo(torneo_id: int, current: CurrentUser, db: DBSession) -> dict:
    ...
    Un partido es "activo" si:
      - Esta dentro de la ventana temporal (15 a 300 min desde inicio)
      - O ya esta marcado como 'en_juego' en BD
      - Y NO tiene datos_confirmados (admin no lo ha blindado todavia)

    IMPORTANTE: NO excluimos por estado='finalizado' aqui porque en partidos KO
    la API-Football puede retornar status 'FT' brevemente al final de los 90 min,
    antes de que empiece el alargue (AET). Si excluimos los partidos 'finalizado'
    de esta ventana, el sync se detiene y nunca captura el alargue ni los penales.

    La ventana empieza en +15 min para evitar calls a la API antes de que haya
    datos disponibles. 300 min cubre 90 min + alargue (30) + penales (~30) + margen.

    Usado por sync_auto.py: si activo=false, el script sale sin llamar a API-Football.
    """
    if not await _check_admin(current):
        raise HTTPException(403, "Se requiere rol admin o superadmin")
    ...
    r = await db.execute(
        text("""
            ...
            WHERE p.torneo_id = :tid
              AND p.fecha IS NOT NULL
              -- Excluir solo partidos que el admin ya confirmo (Blindar).
              -- NO excluir por estado='finalizado': un partido KO puede pasar
              -- brevemente por status FT antes de entrar al alargue (AET).
              -- Si lo excluimos aqui, el sync se detiene y no captura ET/P/PEN.
              AND NOT COALESCE(p.datos_confirmados, FALSE)
              AND (
                -- Ventana temporal (min 15 a 300 desde inicio)
                -- Incluye partidos en cualquier estado (programado/en_juego/finalizado-transitorio)
    ...
    for row in r.mappings():
        d = dict(row)
        if d.get("fecha"):
            d["fecha"] = d["fecha"].strftime("%Y-%m-%dT%H:%M:%SZ") if hasattr(d["fecha"], "strftime") else str(d["fecha"])
        partidos.append(d)
...
async def _calc_standings_reales(db: DBSession, torneo_id: int) -> dict:
    ...
    for p in r_parts.mappings():
        ...
        # Acumular por separado para poblar columnas fuente del generated column
        loc["amarillas_total"] += p["local_amarillas"]
        loc["rojas_total"]     += p["local_rojas"]
        vis["amarillas_total"] += p["visitante_amarillas"]
        vis["rojas_total"]     += p["visitante_rojas"]
            ...
            await db.execute(
                text("""
                    ...
                    SET pj=:pj, pg=:pg, pe=:pe, pp=:pp,
                        ...
                        amarillas=:am, rojas_directas=:rd, rojas_doble_amarilla=0
                ...
                """),
                {
                    ...
                    "am": eq.get("amarillas_total", 0),
                    "rd": eq.get("rojas_total", 0),
...
async def leer_apuestas_globales(
    ...
    ag_dict = dict(ag) if ag else {}
    pg_dict = dict(pg) if pg else {}

    # --- Nombres de equipos referenciados en las predicciones ---
    team_ids = {
        ag_dict.get("pred_campeon_id"), ag_dict.get("pred_finalista1_id"),
        ag_dict.get("pred_finalista2_id"), ag_dict.get("pred_peor_equipo_id"),
    }
    team_ids.discard(None)
    nombres_eq: dict = {}
    if team_ids:
        ids_sql = ",".join(str(i) for i in team_ids)
        rn = await db.execute(text(f"SELECT id, COALESCE(nombre_es, nombre) AS nombre FROM equipo WHERE id IN ({ids_sql})"))
        nombres_eq = {row["id"]: row["nombre"] for row in rn.mappings()}

    # --- Resultados reales del torneo (A-D, para mostrar en la ficha) ---
    resultados: dict = {}
    # A/B: campeon y finalistas (partido final jugado)
    try:
        rf = await db.execute(text("""
            SELECT p.equipo_local_id, p.equipo_visitante_id, p.equipo_clasificado_id,
                   COALESCE(el.nombre_es, el.nombre) AS local_nombre,
                   COALESCE(ev.nombre_es, ev.nombre) AS visit_nombre,
                   COALESCE(ec.nombre_es, ec.nombre) AS campeon_nombre
            FROM partido p
            JOIN fase f ON f.id = p.fase_id
            LEFT JOIN equipo el ON el.id = p.equipo_local_id
            LEFT JOIN equipo ev ON ev.id = p.equipo_visitante_id
            LEFT JOIN equipo ec ON ec.id = p.equipo_clasificado_id
            WHERE p.torneo_id = :tid AND f.tipo = 'final' AND p.estado = 'finalizado'
            LIMIT 1
        """), {"tid": torneo_id})
        final = rf.mappings().first()
        if final:
            resultados["campeon_id"]        = final["equipo_clasificado_id"]
            resultados["campeon_nombre"]    = final["campeon_nombre"]
            resultados["finalista1_id"]     = final["equipo_local_id"]
            resultados["finalista1_nombre"] = final["local_nombre"]
            resultados["finalista2_id"]     = final["equipo_visitante_id"]
            resultados["finalista2_nombre"] = final["visit_nombre"]
    except Exception:
        pass

    # C + D: goleador y peor equipo (admin-set en tabla torneo)
    try:
        rt = await db.execute(
            text("SELECT resultado_goleador, resultado_peor_equipo_id FROM torneo WHERE id = :tid"),
            {"tid": torneo_id},
        )
        rt_row = rt.mappings().first() or {}
        resultados["goleador"] = rt_row.get("resultado_goleador")
        peor_id = rt_row.get("resultado_peor_equipo_id")
        if peor_id:
            rpe = await db.execute(
                text("SELECT COALESCE(nombre_es, nombre) AS nombre FROM equipo WHERE id = :id"),
                {"id": peor_id},
            )
            peor_row = rpe.mappings().first()
            resultados["peor_equipo_id"]     = peor_id
            resultados["peor_equipo_nombre"] = (peor_row or {}).get("nombre")
    except Exception:
        pass

    # E: mayor goleada (solo cuando hay campeón)
    try:
        if resultados.get("campeon_id"):
            rg = await db.execute(text("""
                ...
                LIMIT 1
            """), {"tid": torneo_id})
            mg = rg.mappings().first()
            if mg:
                g, p = max(mg["goles_local"], mg["goles_visitante"]), min(mg["goles_local"], mg["goles_visitante"])
                resultados["goleada_str"] = f"{g}–{p}"
    except Exception:
        pass

    # F + G: etapa y goles de Paraguay (cuando Paraguay no tiene KO pendientes)
    try:
        py_r = await db.execute(text("""
            SELECT id FROM equipo
            WHERE LOWER(nombre) LIKE '%paraguay%' OR LOWER(nombre_es) LIKE '%paraguay%'
            LIMIT 5
        """))
        py_ids = [r["id"] for r in py_r.mappings()]
        if py_ids:
            ids_sql = ",".join(str(i) for i in py_ids)
            done_r = await db.execute(text(f"""
                SELECT
                  SUM(CASE WHEN p.estado='finalizado' AND f.tipo NOT ILIKE 'grupo%%' THEN 1 ELSE 0 END) AS ko_fin,
                  SUM(CASE WHEN p.estado NOT IN ('finalizado','aplazado','cancelado') AND f.tipo NOT ILIKE 'grupo%%' THEN 1 ELSE 0 END) AS ko_pend
                FROM partido p JOIN fase f ON f.id=p.fase_id
                WHERE p.torneo_id=:tid AND p.numero_fifa IS NOT NULL
                  AND (p.equipo_local_id IN ({ids_sql}) OR p.equipo_visitante_id IN ({ids_sql}))
            """), {"tid": torneo_id})
            dr = done_r.mappings().first()
            py_done = (dr["ko_fin"] or 0) > 0 and (dr["ko_pend"] or 0) == 0
            if py_done:
                _orden = {"final":7,"tercer_puesto":6,"semis":5,"cuartos":4,"ronda16":3,"ronda32":2,"grupo":1}
                _label = {"final":"Final","tercer_puesto":"3er puesto","semis":"Semifinales",
                          "cuartos":"Cuartos de final","ronda16":"Ronda de 16","ronda32":"Ronda de 32","grupo":"Fase de grupos"}
                _inv = {v: k for k, v in _orden.items()}
                py_r2 = await db.execute(text(f"""
                    SELECT f.tipo AS fase_tipo,
                           CASE WHEN p.equipo_local_id IN ({ids_sql}) THEN p.goles_local
                                WHEN p.equipo_visitante_id IN ({ids_sql}) THEN p.goles_visitante ELSE 0 END AS goles_py
                    FROM partido p JOIN fase f ON f.id=p.fase_id
                    WHERE p.torneo_id=:tid AND p.estado='finalizado' AND p.numero_fifa IS NOT NULL
                      AND (p.equipo_local_id IN ({ids_sql}) OR p.equipo_visitante_id IN ({ids_sql}))
                """), {"tid": torneo_id})
                rows = py_r2.mappings().all()
                if rows:
                    resultados["goles_paraguay"] = sum((r["goles_py"] or 0) for r in rows)
                    max_ord = max(_orden.get(r["fase_tipo"], 0) for r in rows)
                    etapa_key = _inv.get(max_ord)
                    resultados["etapa_paraguay"] = _label.get(etapa_key, etapa_key)
    except Exception:
        pass

    # ── Ganadores por ítem (apostadores con pts > 0) ──────────────
    ganadores: dict = {}
    try:
        item_cols = [
            ("pts_campeon", "A"), ("pts_finalistas", "B"), ("pts_goleador", "C"),
            ("pts_peor_equipo", "D"), ("pts_mayor_goleada", "E"),
            ("pts_etapa_paraguay", "F"), ("pts_goles_paraguay", "G"),
        ]
        rw = await db.execute(
            text("""
                SELECT apostador_id,
                       pts_campeon, pts_finalistas, pts_goleador, pts_peor_equipo,
                       pts_mayor_goleada, pts_etapa_paraguay, pts_goles_paraguay
                FROM puntaje_global WHERE torneo_id = :tid
            """),
            {"tid": torneo_id},
        )
        pg_rows = rw.mappings().all()
        # Recopilar IDs ganadores por columna
        winners_ids: dict = {}  # col -> [uid]
        for row in pg_rows:
            for col, letter in item_cols:
                val = row.get(col)
                if val and val > 0:
                    winners_ids.setdefault(col, []).append(row["apostador_id"])
        # Resolver aliases desde app_db
        all_ids = list({uid for uids in winners_ids.values() for uid in uids})
        alias_map: dict = {}
        if all_ids:
            async with _app_engine.connect() as conn:
                ur = await conn.execute(
                    text("SELECT id, COALESCE(apostador, username) AS alias FROM users WHERE id = ANY(:ids)"),
                    {"ids": all_ids},
                )
                alias_map = {r["id"]: r["alias"] for r in ur.mappings()}
        for col, letter in item_cols:
            uids = winners_ids.get(col, [])
            ganadores[letter] = [alias_map.get(uid, f"?{uid}") for uid in uids]
    except Exception:
        pass

    return {
        "apuesta_global":  ag_dict,
        "puntaje_global":  pg_dict,
        "nombres_equipos": nombres_eq,
...
async def _refresh_goleadores_cache(db, torneo_id: int) -> int:
    """
    Fuerza actualización de goleadores_cache desde API-Football.
    Borra los registros existentes e inserta los nuevos top-20.
    Llamado automáticamente al calcular puntajes.
    Devuelve la cantidad de goleadores insertados (0 si falla o sin API key).
    """
    ...
    try:
        cfg_row = (await db.execute(text("""
            ...
            WHERE t.id = :tid
        """), {"tid": torneo_id})).mappings().first()
        if not cfg_row or not cfg_row["api_league_id"]:
            return 0
        from app.core.config import settings as _cfg_gc
        if not _cfg_gc.APIFOOTBALL_KEY:
            return 0
        async with _httpx_gc.AsyncClient(timeout=15) as _cli:
            _resp = await _cli.get(
                "https://v3.football.api-sports.io/players/topscorers",
                params={"league": cfg_row["api_league_id"], "season": cfg_row["api_season"] or 2026},
                headers={"x-apisports-key": _cfg_gc.APIFOOTBALL_KEY},
            )
        if _resp.status_code != 200:
            return 0
        scorers = (_resp.json().get("response") or [])[:20]
        if not scorers:
            return 0

        # Goles desde eventos_api de BD (más actualizados que API-Football topscorers)
        import json as _json_gc
        db_goles_gc: dict[str, int] = {}
        try:
            _fin_rows = (await db.execute(text("""
                SELECT eventos_api FROM partido
                WHERE fase_id IN (SELECT id FROM fase WHERE torneo_id = :tid)
                  AND estado = 'finalizado'
                  AND eventos_api IS NOT NULL
            """), {"tid": torneo_id})).fetchall()
            for _fr in _fin_rows:
                _evs = _fr[0]
                if isinstance(_evs, str):
                    _evs = _json_gc.loads(_evs or '[]')
                for _ev in (_evs or []):
                    _tipo = (_ev.get("type") or "").lower()
                    _det  = (_ev.get("detail") or "").lower()
                    _pnm  = (_ev.get("player", {}).get("name") or "").strip().lower()
                    if ("goal" in _tipo and "missed" not in _det
                            and "disallow" not in _det and "own" not in _det and _pnm):
                        db_goles_gc[_pnm] = db_goles_gc.get(_pnm, 0) + 1
        except Exception:
            pass

        # Borrar cache existente y reinsertar
        await db.execute(text("DELETE FROM goleadores_cache WHERE torneo_id = :tid"), {"tid": torneo_id})
        inserted = 0
        for i, item in enumerate(scorers):
            p = item.get("player", {}); s = (item.get("statistics") or [{}])[0]
            api_goals = (s.get("goals") or {}).get("total") or 0
            # Buscar goles en BD por nombre del jugador
            skey = (p.get("name") or "").strip().lower()
            db_goals_val = 0
            for dkey, dcnt in db_goles_gc.items():
                if dkey in skey or skey in dkey or any(part in skey for part in dkey.split() if len(part) > 3):
                    db_goals_val = dcnt
                    break
            goles_final = max(api_goals, db_goals_val)
            try:
                await db.execute(text("""
                    INSERT INTO goleadores_cache
                        (torneo_id, jugador_id, nombre, foto_url, equipo_nombre, equipo_logo,
                         goles, asistencias, posicion, actualizado_at)
                    VALUES (:tid,:jid,:nombre,:foto,:eq_nom,:eq_logo,:goles,:asist,:pos,NOW())
                    ON CONFLICT (torneo_id, jugador_id) DO UPDATE SET
                        nombre=EXCLUDED.nombre, foto_url=EXCLUDED.foto_url,
                        equipo_nombre=EXCLUDED.equipo_nombre, equipo_logo=EXCLUDED.equipo_logo,
                        goles=EXCLUDED.goles, asistencias=EXCLUDED.asistencias,
                        posicion=EXCLUDED.posicion, actualizado_at=NOW()
                """), {
                    "tid": torneo_id,
                    "jid": p.get("id") or (i + 1),
                    "nombre": p.get("name") or "?",
                    "foto": p.get("photo") or "",
                    "eq_nom": (s.get("team") or {}).get("name") or "",
                    "eq_logo": (s.get("team") or {}).get("logo") or "",
                    "goles": goles_final,
...
async def calcular_puntajes(torneo_id: int, current: CurrentUser, db: DBSession, force_grupos: bool = False) -> dict:
    if not await _check_admin(current):
        raise HTTPException(403, "Se requiere rol admin o superadmin")
    ...
    # (se omite si force_grupos=True para forzar recálculo de grupos bloqueados)
    _grupos_auto_bloqueadas = 0 if force_grupos else await _auto_lock_completed_grupos(db, torneo_id)
    ...
    result = await calc.calculate(torneo_id, engine, force_grupos=force_grupos)
    if result is None:
        # Todas las fases con partidos finalizados están bloqueadas (grupos + KO ya cerrados).
        # Los puntajes bloqueados son correctos; aún podemos recalcular globales A-G.
    ...
    # Actualizar goleadores en caché al recalcular puntajes
    _goleadores_actualizados = 0
    try:
        _goleadores_actualizados = await _refresh_goleadores_cache(db, torneo_id)
    except Exception:
        pass
    ...
    return {
        ...
        "goleadores_actualizados": _goleadores_actualizados,
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# CONSULTA Y SYNC INDIVIDUAL DE PARTIDO
# ─────────────────────────────────────────────────────────────────────────────

@router.get("/consulta-partido/{numero_fifa}", summary="Consultar datos de un partido en API-Football (solo lectura, admin)")
async def consulta_partido(
    numero_fifa: int,
    current: CurrentAdmin,
    db: DBSession,
) -> dict:
    """
    Consulta API-Football para un partido específico (por número FIFA).
    Devuelve estado + todos los ítems del partido (goles, amarillas, rojas,
    VAR, minuto primer gol, penales tanda, penales partido).
    NO modifica la base de datos.
    """
    _check_admin(current)

    # 1. Buscar partido en BD
    row = await db.execute(text("""
        SELECT p.id, p.numero_fifa, p.api_fixture_id, p.datos_confirmados,
               p.estado, p.goles_local, p.goles_visitante,
               p.amarillas, p.rojas, p.decisiones_var,
        ...
        JOIN equipo el ON el.id = p.equipo_local_id
        JOIN equipo ev ON ev.id = p.equipo_visitante_id
        WHERE p.numero_fifa = :n
        LIMIT 1
    """), {"n": numero_fifa})
    p = row.mappings().first()
    if not p:
        raise HTTPException(status_code=404, detail=f"Partido P{numero_fifa} no encontrado")

    bd = {
        "partido_id":      p["id"],
        "numero_fifa":     p["numero_fifa"],
        "equipo_local":    p["equipo_local"],
        "equipo_visitante": p["equipo_visitante"],
        "api_fixture_id":  p["api_fixture_id"],
        "datos_confirmados": p["datos_confirmados"],
        "bd_estado":       p["estado"],
        "bd_goles":        f"{p['goles_local']}-{p['goles_visitante']}",
        "bd_amarillas":    p["amarillas"],
        "bd_rojas":        p["rojas"],
        "bd_var":          p["decisiones_var"],
        "bd_minuto_gol":   p["minuto_primer_gol"],
        "bd_penales_partido": p["penales_partido"],
        "bd_penales_tanda":   f"{p['penales_local']}-{p['penales_visitante']}" if p["penales_local"] is not None else None,
    }

    if not p["api_fixture_id"]:
        return {
            "ok": False,
            "error": "Partido sin api_fixture_id — ejecutar auto-mapeo primero",
            "bd": bd,
            "api": None,
        }

    # 2. Llamar API-Football
    from app.services.sync_api_football import _headers, API_BASE, _update_partido_full
    import httpx
    ...
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            resp = await client.get(
                f"{API_BASE}/fixtures",
                params={"id": p["api_fixture_id"]},
                headers=_headers(),
            )
            resp.raise_for_status()
            data = resp.json()
    except Exception as e:
        return {"ok": False, "error": f"API-Football error: {e}", "bd": bd, "api": None}

    fixtures = data.get("response", [])
    if not fixtures:
        return {"ok": False, "error": "API-Football devolvió respuesta vacía", "bd": bd, "api": None}

    fix = fixtures[0]
    s   = fix["fixture"]["status"]
    STATUS_MAP = {
    ...
    estado_api   = STATUS_MAP.get(status_short, status_short)
    ...
    # Parsear eventos (misma lógica que _update_partido_full)
    events = fix.get("events", [])
    events_sorted = sorted(events, key=lambda e: e.get("time", {}).get("elapsed") or 999)
    amarillas_ev = rojas_ev = var_count = penales_pp = 0
    minuto_gol = None
    goles_anulados: set[int] = set()
    ...
    for ev in events_sorted:
        ...
        if t == "Var":
            var_count += 1
            if d in ("Goal Disallowed","Goal Cancelled","Offside Goal") and elapsed:
                goles_anulados.add(elapsed)
        ...
        elapsed_max = 119 if pen_home is not None else 120
        comments = (ev.get("comments") or "").lower()
        in_play = (
            "shootout" not in comments and
            elapsed is not None and elapsed <= elapsed_max
        )
        if t == "Goal" and d == "Penalty" and in_play:
            penales_pp += 1
        elif t in ("Miss","Goal") and d in ("Missed Penalty","Penalty Missed") and in_play:
            penales_pp += 1

    for ev in events_sorted:
        t, d = ev.get("type",""), ev.get("detail","")
        elapsed = ev.get("time",{}).get("elapsed")
        if t == "Goal" and d not in ("Penalty Missed","Missed Penalty"):
            if elapsed and elapsed not in goles_anulados and minuto_gol is None:
                minuto_gol = elapsed

    # Estadísticas de API
    amar_stats = rojas_stats = None
    for st in fix.get("statistics", []):
        for item in st.get("statistics", []):
            try: v = int(item.get("value") or 0)
            except: v = 0
    ...
    api = {
        ...
        "goles_local":       fix["goals"]["home"],
        "goles_visitante":   fix["goals"]["away"],
        "penales_tanda_local":    pen_home,
        "penales_tanda_visitante": pen_away,
        "amarillas":         amarillas_ev if amarillas_ev > 0 else amar_stats,
        "rojas":             rojas_ev if rojas_ev > 0 else rojas_stats,
        "decisiones_var":    var_count if events else None,
        "penales_partido":   penales_pp if events else None,
        "minuto_primer_gol": minuto_gol,
        "has_data":          has_data,
        "eventos_count":     len(events),
        "cuota_restante":    int(resp.headers.get("x-ratelimit-requests-remaining", -1)),
    }
    ...
    return {
        ...
        "api": api,
        "diferencias": {
            k: {"bd": bd.get(f"bd_{k}"), "api": api.get(k)}
            for k in ("estado","amarillas","rojas","decisiones_var","minuto_primer_gol","penales_partido")
            if str(bd.get(f"bd_{k}")) != str(api.get(k))
        },
    }


@router.post("/sync-partido/{numero_fifa}", summary="Sincronizar un partido específico desde API-Football (admin)")
async def sync_partido_individual(
    numero_fifa: int,
    current: CurrentAdmin,
    db: DBSession,
    force: bool = True,
    ...
    """
    Sincroniza UN partido específico (por número FIFA) desde API-Football.
    Actualiza todos los campos: goles, estado, amarillas, rojas, VAR,
    minuto primer gol, penales tanda, penales partido.
    Al finalizar: avanza bracket KO y recalcula puntajes.
    Si no tiene api_fixture_id, ejecuta auto-mapeo primero.
    """
    _check_admin(current)

    # 1. Verificar que existe y tiene fixture_id
    row = await db.execute(text("""
        SELECT p.id, p.numero_fifa, p.api_fixture_id, p.datos_confirmados,
               p.estado, el.nombre AS equipo_local, ev.nombre AS equipo_visitante,
               f.torneo_id
        FROM partido p
        JOIN equipo el ON el.id = p.equipo_local_id
        JOIN equipo ev ON ev.id = p.equipo_visitante_id
    ...
    if not p["api_fixture_id"]:
        ...
        # Re-leer fixture_id post-mapeo
        row2 = await db.execute(text(
            "SELECT api_fixture_id FROM partido WHERE id = :pid"
        ), {"pid": partido_id})
        fix_id_row = row2.mappings().first()
        fix_id = fix_id_row["api_fixture_id"] if fix_id_row else None
        if not fix_id:
            return {
                "ok": False,
                "error": "No se pudo obtener api_fixture_id (auto-mapeo sin resultado)",
                "mapeo": mapeo_info,
            }
    else:
        fix_id = p["api_fixture_id"]

    # 2. Llamar sync_torneo con max_detalle=1, pasando solo este fixture
    from app.services.sync_api_football import _headers, API_BASE, _update_partido_full
    import httpx
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            resp.raise_for_status()
            fix_data = resp.json().get("response", [])
    except Exception as e:
        return {"ok": False, "error": f"API-Football error: {e}"}

    if not fix_data:
        return {"ok": False, "error": "API-Football no devolvió datos para este fixture"}

    fix = fix_data[0]
    status_short = fix["fixture"]["status"]["short"]

    # 3. Mapear api_team_id → equipo_id para _update_partido_full
    ...
    api_raw = await _update_partido_full(db, partido_id, fix, team_id_map)
    await db.commit()

    # 5. Avanzar bracket y recalcular puntajes si el partido finalizó
    bracket_ok = False
    puntajes_ok = False
    puntajes    = {}

    ESTADOS_FIN = {"FT", "AET", "PEN"}
    if status_short in ESTADOS_FIN:
        try:
        ...
        try:
            ...
            puntajes = await ScoringCalculator(db).calculate(torneo_id, engine)
            await ScoringCalculator(db).calculate_global(torneo_id, engine)
            await db.commit()
            puntajes_ok = True
        except Exception as e:
    ...
    return {
        "ok": True,
        "partido": f"P{numero_fifa} — {p['equipo_local']} vs {p['equipo_visitante']}",
        "api_fixture_id": fix_id,
        "estado_api":  status_short,
        "goles":       f"{fix['goals']['home']}-{fix['goals']['away']}",
        "api_items":   api_raw,
...
@router.get("/scheduler-status", summary="Estado del Task Scheduler BECBUC-SyncAPI (admin)")
async def scheduler_status(current: CurrentAdmin) -> dict:
    """Consulta si la tarea BECBUC-SyncAPI esta registrada y activa en Windows Task Scheduler."""
    _check_admin(current)
    import subprocess
    try:
        ...
        if result.returncode != 0:
            return {"registered": False, "active": False, "status": "No registrada", "detail": result.stderr.strip()[:300]}
        output = result.stdout
        status_line = ""
        for line in output.splitlines():
            lower = line.lower()
            if any(k in lower for k in ("estado de la tarea", "status:", "task state")):
                status_line = line.split(":", 1)[-1].strip()
                break
        active = status_line.lower() in ("ready", "listo", "running", "ejecutando", "en ejecucion")
        return {"registered": True, "active": active, "status": status_line, "detail": output[:600]}
    except Exception as e:
        return {"registered": False, "active": False, "status": "Error", "detail": str(e)}


@router.post("/scheduler-start", summary="Registra/reinicia BECBUC-SyncAPI en Task Scheduler (admin)")
async def scheduler_start(current: CurrentAdmin) -> dict:
    """
    Registra la tarea BECBUC-SyncAPI en Windows Task Scheduler.
    ...
    try:
        ...
        return {
            "ok": ok,
            "message": "Tarea BECBUC-SyncAPI registrada correctamente" if ok else "Error al registrar",
...
async def auto_catchup(
    ...
    Mecanismo de recuperación automática:
      1. Busca partidos FINALIZADOS que tienen api_fixture_id pero:
         - Sin ninguna fila en puntaje_detalle, O
         - Con decisiones_var IS NULL o minuto_primer_gol IS NULL (stats incompletas)
         Y que NO estén blindados (datos_confirmados=FALSE).
      2. Para cada uno: llama a API-Football y actualiza los campos faltantes.
      3. Al final: recalcula puntajes y globales una sola vez.

    Diseñado para ser llamado desde sync_auto.py cada N minutos como catch-up.
    Costo API: 1 call por partido sin datos (usando fixture individual).
    """
    _check_admin(current)

    # 1. Buscar partidos finalizados sin puntaje o con stats incompletas
    r_pending = await db.execute(text("""
        SELECT p.id, p.numero_fifa, p.api_fixture_id,
               p.decisiones_var, p.minuto_primer_gol,
               el.nombre AS local, ev.nombre AS visitante
        FROM partido p
        JOIN fase f ON f.id = p.fase_id
        JOIN equipo el ON el.id = p.equipo_local_id
        ...
        WHERE f.torneo_id = :tid
          ...
          AND (
            NOT EXISTS (
                SELECT 1 FROM puntaje_detalle pd WHERE pd.partido_id = p.id
            )
            OR p.decisiones_var IS NULL
            OR p.minuto_primer_gol IS NULL
          )
        ORDER BY p.fecha
    """), {"tid": torneo_id})
    pending = r_pending.mappings().all()

    if not pending:
        return {
            "ok": True,
            "mensaje": "No hay partidos pendientes de sync/recalculo",
            "catchup_count": 0,
            "puntajes_ok": False,
        }

    # 2. Obtener mapeo de api_team_id → equipo_id
    from app.services.sync_api_football import _headers, API_BASE, _update_partido_full
    import httpx
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for p in pending:
            ...
            numero_fifa = p["numero_fifa"]
            fix_id      = p["api_fixture_id"]
            label       = f"P{numero_fifa} {p['local']} vs {p['visitante']}"
            ...
            try:
                ...
                resp.raise_for_status()
                api_calls += 1
                fix_list = resp.json().get("response", [])
                if not fix_list:
                    errors.append({"partido": label, "error": "API sin response"})
                    continue

                fix = fix_list[0]
                api_items = await _update_partido_full(db, partido_id, fix, team_id_map)
                await db.commit()
                synced.append({
                    "partido": label,
                    "fix_id": fix_id,
    ...
    # 3. Avanzar bracket y recalcular puntajes una sola vez
    bracket_ok  = False
    puntajes_ok = False
    puntajes    = {}
...
async def puntos_por_fase_excel(torneo_id: int, current: CurrentUser, db: DBSession):
    ...
    fname = f"becbuc_puntos_por_fase_torneo_{torneo_id}.xlsx"
...
async def exportar_transparencia(torneo_id: int, current: CurrentUser, db: DBSession):
    ...
    fname = f"becbuc_transparencia_torneo{torneo_id}_{ts}.xlsx"
    return StreamingResponse(
        buf,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'attachment; filename="{fname}"'},
...
async def generar_auditoria(torneo_id: int, current: CurrentUser, db: DBSession) -> dict:
    ...
    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    filename = f"becbuc_auditoria_torneo{torneo_id}_{ts}.xlsx"
...
async def live_panel(
    ...
    if numero_fifa is not None:
        ...
        # Auto: en_juego primero, luego el próximo pendiente/programado preferiendo HOY (CR), luego futuro
        _tz_hoy = "(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date"
        _fecha_hoy = f"DATE(p.fecha AT TIME ZONE 'UTC') = {_tz_hoy}"
        partido_r = await db.execute(
            text(_partido_sql + f"""
                  AND p.estado IN ('programado', 'pendiente', 'en_juego')
                ORDER BY
                    CASE p.estado WHEN 'en_juego' THEN 0 ELSE 1 END,
                    CASE WHEN {_fecha_hoy} THEN 0 ELSE 1 END,
        ...
        partido_row = partido_r.mappings().fetchone()
        if not partido_row:
            # Fallback: ultimo finalizado de HOY primero, luego cualquier finalizado reciente
    ...
    # 2c. Todos los partidos KO de HOY (en_juego + programado/pendiente) para sim-bar
    partidos_hoy = []
    try:
        hoy_r = await db.execute(
            text("""
                ...
                LEFT JOIN equipo ev ON ev.id = p.equipo_visitante_id
                WHERE p.torneo_id = :tid
                  AND f.tipo NOT ILIKE 'grupo%%'
                  AND p.estado NOT IN ('aplazado', 'cancelado')
                  AND DATE(p.fecha AT TIME ZONE 'UTC') = (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date
                ORDER BY p.fecha ASC NULLS LAST
            """),
            {"tid": torneo_id},
        )
        for _r in hoy_r.mappings().fetchall():
            _d = dict(_r)
            if _d.get("fecha") and isinstance(_d["fecha"], _dtp):
                _d["fecha"] = _d["fecha"].strftime("%Y-%m-%dT%H:%M:%SZ")
            partidos_hoy.append(_d)
    except Exception as _e_hoy:
        print(f"[live_panel] partidos_hoy query error (non-fatal): {_e_hoy}")
...
@router.post("/refresh-goleadores/{torneo_id}", summary="Fuerza actualización de goleadores desde API-Football (admin)")
async def refresh_goleadores(torneo_id: int, current: CurrentUser, db: DBSession) -> dict:
    """Fuerza la actualización de la caché de goleadores desde API-Football. Consume 1 llamada API."""
    if not await _check_admin(current):
        raise HTTPException(403, "Se requiere rol admin o superadmin")
    insertados = await _refresh_goleadores_cache(db, torneo_id)
    return {"ok": insertados > 0, "goleadores_actualizados": insertados}


@router.get("/goleadores-cached/{torneo_id}", summary="Goleadores con caché BD + conteo apostadores")
async def goleadores_cached(torneo_id: int, current: CurrentUser, db: DBSession) -> dict:
    """
    Top goleadores desde caché en BD (TTL 2h). Incluye conteo de apostadores por jugador.
    Si la caché está vacía, llama API-Football una vez para poblarla.
    """
    import httpx as _httpx2
    # Crear tabla caché si no existe (via conexión separada para evitar conflictos de transacción)
    try:
        from sqlalchemy import event as _sa_event
        from app.db.session import engine as _main_engine
        async with _main_engine.begin() as _ddl_conn:
            await _ddl_conn.execute(text("""
        ...
        pass  # tabla ya existe o error DDL — continuamos igual

    # Leer caché < 2h
    cache_rows = (await db.execute(text("""
        SELECT jugador_id, nombre, foto_url, equipo_nombre, equipo_logo,
               goles, asistencias, posicion
        FROM goleadores_cache
        WHERE torneo_id = :tid AND actualizado_at > NOW() - INTERVAL '2 hours'
        ORDER BY posicion LIMIT 20
    """), {"tid": torneo_id})).mappings().all()

    goleadores = [dict(r) for r in cache_rows]

    # Si no hay caché, poblar desde API-Football
    if not goleadores:
        try:
            cfg_row = (await db.execute(text("""
                SELECT t.api_season, c.api_league_id
                ...
                WHERE t.id = :tid
            """), {"tid": torneo_id})).mappings().first()
            if cfg_row and cfg_row["api_league_id"]:
                from app.core.config import settings as _cfg
                if _cfg.APIFOOTBALL_KEY:
                    async with _httpx2.AsyncClient(timeout=15) as _cli:
                        _resp = await _cli.get(
                            "https://v3.football.api-sports.io/players/topscorers",
                            params={"league": cfg_row["api_league_id"], "season": cfg_row["api_season"] or 2026},
                            headers={"x-apisports-key": _cfg.APIFOOTBALL_KEY},
                        )
                    if _resp.status_code == 200:
                        for i, item in enumerate((_resp.json().get("response") or [])[:20]):
                            p = item.get("player", {}); s = (item.get("statistics") or [{}])[0]
                            try:
                                await db.execute(text("""
                                    INSERT INTO goleadores_cache
                                        (torneo_id, jugador_id, nombre, foto_url, equipo_nombre, equipo_logo, goles, asistencias, posicion, actualizado_at)
                                    VALUES (:tid,:jid,:nombre,:foto,:eq_nom,:eq_logo,:goles,:asist,:pos,NOW())
                                    ON CONFLICT (torneo_id, jugador_id) DO UPDATE SET
                                        nombre=EXCLUDED.nombre, goles=EXCLUDED.goles,
                                        asistencias=EXCLUDED.asistencias, posicion=EXCLUDED.posicion,
                                        actualizado_at=NOW()
                                """), {
                                    "tid": torneo_id,
                                    "jid": p.get("id") or (i + 1),
                                ...
                                pass
                        await db.commit()
                        cache_rows2 = (await db.execute(text("""
                            SELECT jugador_id, nombre, foto_url, equipo_nombre, equipo_logo,
                                   goles, asistencias, posicion
                            FROM goleadores_cache WHERE torneo_id = :tid ORDER BY posicion LIMIT 20
                        """), {"tid": torneo_id})).mappings().all()
                        goleadores = [dict(r) for r in cache_rows2]
        except Exception:
            pass

    # Predicciones apostadores
    preds_raw = (await db.execute(text("""
        SELECT apostador_id, pred_goleador FROM apuesta_global
        WHERE torneo_id = :tid AND pred_goleador IS NOT NULL AND pred_goleador != ''
    """), {"tid": torneo_id})).mappings().all()

    res_row = (await db.execute(text("SELECT resultado_goleador FROM torneo WHERE id = :tid"), {"tid": torneo_id})).first()
    resultado_goleador = (res_row[0] or "").strip() if res_row else ""

    pred_ids = list({p["apostador_id"] for p in preds_raw})
    alias_map_c: dict = {}
    if pred_ids:
        try:
            async with _app_engine.connect() as _aconn:
                _ur = await _aconn.execute(
                    text("SELECT id, COALESCE(apostador, username) AS alias FROM users WHERE id = ANY(:ids)"),
                    {"ids": pred_ids}
                )
                alias_map_c = {row["id"]: row["alias"] for row in _ur.mappings()}
        except Exception:
            pass

    # Contar apostadores por jugador (substring match)
    for g in goleadores:
        jname = (g.get("nombre") or "").lower()
        jparts = [p.strip() for p in jname.split() if len(p.strip()) > 2]
        matches = []
        for p in preds_raw:
            pred_lower = (p["pred_goleador"] or "").lower()
            if any(part in pred_lower or pred_lower in part for part in jparts if part):
                matches.append(alias_map_c.get(p["apostador_id"], "?"))
        g["apostadores"] = {"count": len(matches), "aliases": sorted(set(matches))}
        # Serializar campos timestamp si existen
        if "actualizado_at" in g and hasattr(g["actualizado_at"], "isoformat"):
            g["actualizado_at"] = g["actualizado_at"].isoformat()

    return {
        "goleadores": goleadores,
        "resultado_goleador": resultado_goleador,
        "total_predicciones": len(preds_raw),
    }


@router.get("/goleadores/{torneo_id}", summary="Top goleadores del torneo + predicciones apostadores")
async def get_goleadores(torneo_id: int, current: CurrentUser, db: DBSession) -> dict:
    """
    Retorna tabla de goleadores desde API-Football (/players/topscorers)
    y las predicciones de cada apostador para el ítem C (goleador).
    """
    import httpx as _httpx
    ...
    api_season    = cfg["api_season"] or 2026
    api_league_id = cfg["api_league_id"] or 1   # 1 = Copa Mundial FIFA

    # 2. Predicciones de todos los apostadores (pred_goleador es texto libre)
    # Predicciones de todos los apostadores
    preds_raw = (await db.execute(text("""
        SELECT apostador_id, pred_goleador
        FROM apuesta_global
        WHERE torneo_id = :tid AND pred_goleador IS NOT NULL AND pred_goleador != ''
        ORDER BY pred_goleador
    """), {"tid": torneo_id})).mappings().all()

    # Obtener aliases desde app_db
    pred_ids = list({p["apostador_id"] for p in preds_raw})
    alias_map_g: dict[int, str] = {}
    if pred_ids:
        async with _app_engine.connect() as _aconn:
            _ur = await _aconn.execute(
                text("SELECT id, username FROM users WHERE id = ANY(:ids)"),
            ...
            for _row in _ur.mappings():
                alias_map_g[_row["id"]] = _row["username"] or "?"

    preds_rows = [
        {"apostador_id": p["apostador_id"],
         "pred_goleador": p["pred_goleador"],
         "alias": alias_map_g.get(p["apostador_id"], "?")}
        for p in preds_raw
    ]

    # Agrupar predicciones por nombre normalizado (lowercase)
    pred_map: dict[str, list[dict]] = {}
    for p in preds_rows:
        key = (p["pred_goleador"] or "").strip().lower()
        if key:
            ...
            pred_map[key].append({
                "apostador_id": p["apostador_id"],
                "alias": p["alias"],
                "pred": p["pred_goleador"],
            })

    # Predicción del usuario actual
    my_pred_row = (await db.execute(text("""
        SELECT pred_goleador FROM apuesta_global
        WHERE torneo_id = :tid AND apostador_id = :uid
    """), {"tid": torneo_id, "uid": current.id})).first()
    my_pred = (my_pred_row[0] or "").strip() if my_pred_row else ""

    # Resultado oficial si ya está registrado
    res_row = (await db.execute(text("""
        SELECT resultado_goleador FROM torneo WHERE id = :tid
    """), {"tid": torneo_id})).first()
    resultado_goleador = (res_row[0] or "").strip() if res_row else ""

    # 3. Llamar API-Football
    _settings = __import__('app.core.config', fromlist=['settings']).settings
    if not _settings.APIFOOTBALL_KEY:
        return {
            "ok": False,
            "error": "APIFOOTBALL_KEY no configurada",
            "my_pred": my_pred,
            "resultado_goleador": resultado_goleador,
            "predicciones": pred_map,
            "scorers": [],
        }
    ...
    try:
        async with _httpx.AsyncClient(timeout=15) as client:
            resp = await client.get(
                ...
                params={"league": api_league_id, "season": api_season},
                headers={
                    "x-rapidapi-key": _settings.APIFOOTBALL_KEY,
        ...
        scorers = []
        for item in raw[:20]:  # Top 20 máximo
            pl = item.get("player", {})
            st = (item.get("statistics") or [{}])[0]
            goals = st.get("goals", {})
            name = pl.get("name", "")
            key = name.strip().lower()
            # Buscar si algún apostador predijo este jugador
            # Match aproximado: si el nombre del apostador está contenido en el nombre real o viceversa
            matching_preds = []
            for pk, pv in pred_map.items():
                if pk in key or key in pk or any(part in key for part in pk.split() if len(part) > 3):
                    matching_preds = pv
                    break
            ...
            scorers.append({
                ...
                "team_logo": st.get("team", {}).get("logo", ""),
                "goals": goals.get("total") or 0,
                "assists": goals.get("assists") or 0,
                "penalties": goals.get("penalty") or 0,
                "apostadores_pred": matching_preds,
            })

        # Goles provisionales del partido en vivo
        live_goles_por_jugador: dict[str, int] = {}
        live_partido_info: dict = {}
        try:
            live_r = await db.execute(text("""
                SELECT p.id, p.eventos_api,
                       COALESCE(el.nombre_es, el.nombre, '?') AS local_nm,
                       COALESCE(ev.nombre_es, ev.nombre, '?') AS visitante_nm,
                       f.nombre AS fase_nm
                FROM partido p
                LEFT JOIN equipo el ON el.id = p.equipo_local_id
                LEFT JOIN equipo ev ON ev.id = p.equipo_visitante_id
                ...
                WHERE p.estado = 'en_juego'
                  AND p.fase_id IN (SELECT id FROM fase WHERE torneo_id = :tid)
                LIMIT 1
            """), {"tid": torneo_id})
            live_row = live_r.mappings().first()
            if live_row and live_row["eventos_api"]:
                eventos = live_row["eventos_api"]
                if isinstance(eventos, str):
                    import json as _json
                    eventos = _json.loads(eventos)
                live_partido_info = {
                    "local": live_row["local_nm"],
                    "visitante": live_row["visitante_nm"],
                    "fase": live_row["fase_nm"],
                }
                for ev_item in (eventos or []):
                    tipo = (ev_item.get("type") or "").lower()
                    detalle = (ev_item.get("detail") or "").lower()
                    jugador = ev_item.get("player", {}).get("name") or ""
                    if "goal" in tipo and "missed" not in detalle and "disallow" not in detalle:
                        key = jugador.strip().lower()
                        if key:
                            live_goles_por_jugador[key] = live_goles_por_jugador.get(key, 0) + 1
        except Exception:
            pass

        # Goles desde eventos_api de TODOS los partidos finalizados en BD
        # (más actualizado que API-Football topscorers que puede tener delay)
        db_goles_por_jugador: dict[str, int] = {}
        try:
            fin_rows = await db.execute(text("""
                ...
                WHERE fase_id IN (SELECT id FROM fase WHERE torneo_id = :tid)
                  ...
                  AND eventos_api IS NOT NULL
            """), {"tid": torneo_id})
            import json as _json_gl
            for _fr in fin_rows:
                _evs = _fr[0]
                if isinstance(_evs, str):
                    _evs = _json_gl.loads(_evs or '[]')
                for _ev in (_evs or []):
                    _tipo = (_ev.get("type") or "").lower()
                    _det  = (_ev.get("detail") or "").lower()
                    _pnm  = (_ev.get("player", {}).get("name") or "").strip().lower()
                    # contar goles reales (excluir penales fallados, goles anulados, autogoles)
                    if ("goal" in _tipo and "missed" not in _det
                            and "disallow" not in _det and "own" not in _det and _pnm):
                        db_goles_por_jugador[_pnm] = db_goles_por_jugador.get(_pnm, 0) + 1
        except Exception:
            pass

        # Agregar provisional_goals (live) y db_goals a cada scorer
        for s in scorers:
            skey = s["name"].strip().lower()
            # Goles en vivo (partido actualmente en_juego)
            prov = 0
            for lkey, lcnt in live_goles_por_jugador.items():
                if lkey in skey or skey in lkey or any(p in skey for p in lkey.split() if len(p) > 3):
                    prov = lcnt
                    break
            # Goles desde BD (todos los finalizados)
            db_goals = 0
            for dkey, dcnt in db_goles_por_jugador.items():
                if dkey in skey or skey in dkey or any(p in skey for p in dkey.split() if len(p) > 3):
                    db_goals = dcnt
                    break
            s["provisional_goals"] = prov
            s["goals_api"]  = s["goals"]         # guardar el valor original de API-Football
            s["goals_db"]   = db_goals            # valor calculado desde eventos BD
            # Usar el mayor de (API-Football, BD, live) como total definitivo
            s["goals"]      = max(s["goals"], db_goals)
            s["total_con_vivo"] = s["goals"] + prov
        ...
        return {
            ...
            "predicciones": pred_map,
            "scorers": scorers,
            "live_partido": live_partido_info,
            "live_goles": live_goles_por_jugador,
            "db_goles": db_goles_por_jugador,
            "api_calls_used": 1,
        }
...
# ── In-memory: mensajes live por partido_id ──────────────────────────────────
_live_mensajes: dict = {}


# ── Live mensajes (En Vivo portal) ──────────────────────────────────────────

@router.post("/live-mensaje/{partido_id}")
async def post_live_mensaje(partido_id: int, body: dict, current: CurrentAdmin, db: DBSession):
    import datetime as _dt
    msg = {
        "id": len(_live_mensajes.get(partido_id, [])) + 1,
        "contenido": str(body.get("contenido", ""))[:300],
        "autor_nombre": getattr(current, "username", None) or getattr(current, "nombre", None) or "Admin",
        "minuto": body.get("minuto"),
        "created_at": _dt.datetime.utcnow().isoformat()
    }
    _live_mensajes.setdefault(partido_id, []).append(msg)
    return {"ok": True, "msg": msg}


@router.get("/live-mensaje/{partido_id}")
async def get_live_mensajes(partido_id: int, current: OptionalCurrentUser = None, db: DBSession = None):
    return {"mensajes": _live_mensajes.get(partido_id, [])}


# ── Admin: partidos-stats (Items x partido en Monitoreo) ─────────────────────

@router.get("/stats-computo/{torneo_id}", summary="Partidos jugados vs computados para el puntaje")
async def stats_computo(torneo_id: int, db: DBSession) -> dict:
    """Devuelve cuántos partidos están jugados (finalizado) y cuántos tienen puntaje calculado."""
    try:
        r = await db.execute(text("""
            SELECT
                COUNT(DISTINCT p.id) FILTER (WHERE p.estado = 'finalizado'
                    AND el.nombre NOT IN ('Por Definir','TBD')
                    AND ev.nombre NOT IN ('Por Definir','TBD')) AS jugados,
                COUNT(DISTINCT pd.partido_id) AS con_puntaje
            FROM partido p
            JOIN fase f ON p.fase_id = f.id
            JOIN equipo el ON el.id = p.equipo_local_id
            JOIN equipo ev ON ev.id = p.equipo_visitante_id
            LEFT JOIN puntaje_detalle pd ON pd.partido_id = p.id AND pd.torneo_id = :tid
            WHERE f.torneo_id = :tid
        """), {"tid": torneo_id})
        row = dict(r.mappings().first() or {})
        return {
            "jugados": int(row.get("jugados") or 0),
            "con_puntaje": int(row.get("con_puntaje") or 0)
        }
    except Exception:
        await db.rollback()
        return {"jugados": 0, "con_puntaje": 0}


@router.get("/admin/partidos-stats/{torneo_id}")
async def admin_partidos_stats(torneo_id: int, current: CurrentAdmin, db: DBSession):
    sql = text("""
        SELECT p.id, p.numero_fifa, p.goles_local, p.goles_visitante, p.estado,
               ...
               f.nombre AS fase_nombre, f.tipo AS fase_tipo,
               COALESCE(f.bloqueada, FALSE) AS fase_bloqueada
        FROM partido p
        JOIN equipo el ON el.id = p.equipo_local_id
        JOIN equipo ev ON ev.id = p.equipo_visitante_id
        JOIN fase f ON f.id = p.fase_id
        WHERE p.torneo_id = :tid AND p.estado = 'finalizado'
          AND el.nombre NOT IN ('Por Definir','TBD')
          AND COALESCE(f.bloqueada, FALSE) = FALSE
        ORDER BY p.numero_fifa DESC
    """)
    rows = (await db.execute(sql, {"tid": torneo_id})).fetchall()
    result = []
    for r in rows:
        result.append({
            "id": r.id, "num": r.numero_fifa,
            "local": r.nombre_local, "visitante": r.nombre_visitante,
            "gl": r.goles_local, "gv": r.goles_visitante,
            "fase": r.fase_nombre, "tipo": r.fase_tipo,
            "amarillas": r.amarillas, "rojas": r.rojas,
            "var": r.decisiones_var, "pp": r.penales_partido,
            "minuto": r.minuto_primer_gol,
            "pen_l": r.penales_local, "pen_v": r.penales_visitante,
            "confirmado": bool(r.datos_confirmados)
        })
    return {"partidos": result}


@router.patch("/admin/partido-stats/{partido_id}")
async def admin_patch_partido_stats(partido_id: int, body: dict, current: CurrentAdmin, db: DBSession):
    allowed = {"amarillas","rojas","decisiones_var","penales_partido","minuto_primer_gol","penales_local","penales_visitante","goles_local","goles_visitante","datos_confirmados"}
    updates = {k: v for k, v in body.items() if k in allowed and v is not None}
    if not updates:
        return {"ok": False, "msg": "No hay campos validos"}
    set_clause = ", ".join(f"{k} = :{k}" for k in updates)
    updates["pid"] = partido_id
    await db.execute(text(f"UPDATE partido SET {set_clause} WHERE id = :pid"), updates)
    await db.commit()
    return {"ok": True, "updated": list(updates.keys())}


# ── Auditoría Excel: comparar Excel subido vs BD ─────────────────────────────

@router.post("/admin/auditoria-diff/{torneo_id}")
async def auditoria_diff(
    torneo_id: int,
    ...
    import io, openpyxl as _opx, re as _re, datetime as _dt
    from fastapi.responses import StreamingResponse as _SR

    content = await file.read()
    try:
        wb_in = _opx.load_workbook(io.BytesIO(content), data_only=True)
    except Exception as e:
        raise HTTPException(400, f"No se pudo leer el Excel: {e}")

    # Cargar partidos de BD
    bd_rows = (await db.execute(text("""
        SELECT p.numero_fifa AS num, p.goles_local AS gl, p.goles_visitante AS gv,
               COALESCE(p.amarillas, 0) AS bd_am,
               COALESCE(p.rojas, 0) AS bd_ro,
               COALESCE(p.decisiones_var, 0) AS bd_var,
        ...
        JOIN equipo ev ON ev.id = p.equipo_visitante_id
        JOIN fase f ON f.id = p.fase_id
        WHERE p.torneo_id = :tid AND p.goles_local IS NOT NULL
        ORDER BY p.numero_fifa
    """), {"tid": torneo_id})).fetchall()
    bd_map = {r.num: r for r in bd_rows}

    # Parsear Excel: buscar celdas con formato P{N}
    # Soporta dos layouts:
    #   A) Transparencia clásica: P{N} | local | gl | gv | visitante  (rest[2]/rest[3] = int)
    #   B) "becbuc audit":        P{N} | match_desc | fase | grupo | alias | alias | loc | loc_s
    #                              | pl | "vs" | pv | vis | ... | txt_real("1-2") → rest[19]
    PAT = _re.compile(r'^P\s*0*(\d{1,3})$', _re.IGNORECASE)
    PAT_SCORE = _re.compile(r'^(\d+)\s*[–\-]\s*(\d+)$')
    excel_partidos = {}
    excel_stats    = {}  # num -> {am, ro, var_, min} from TBL MASTER RESULTADOS

    def _toint(v):
        try: return int(float(str(v)))
        except: return None

    def _parse_score_txt(v):
        """Parsea "1-2", "1–2" o "Team N vs M Team" → (N, M), None si falla."""
        if v is None: return None, None
        s = str(v).strip()
        # Formato "1-2" o "1–2"
        m2 = PAT_SCORE.match(s)
        if m2: return int(m2.group(1)), int(m2.group(2))
        # Formato "P001: Mexico 2 vs 0 South Africa" (TXT RESULTADO FINAL de becbuc audit)
        m3 = _re.search(r'\b(\d+)\s+vs\s+(\d+)\b', s, _re.IGNORECASE)
        if m3: return int(m3.group(1)), int(m3.group(2))
        return None, None
    ...
    for sheet_name in wb_in.sheetnames:
        ...
        for row in ws.iter_rows(values_only=True):
            for ci, cell in enumerate(row):
                ...
                m = PAT.match(str(cell).strip())
                if not m: continue
                num = int(m.group(1))
                if not (1 <= num <= 104): continue
                # Extender slice a 40 para cubrir layout "becbuc audit"
                rest = list(row[ci:ci+40]) + [None]*40

                # Layout A: goles directos en rest[2] y rest[3]
                gl = _toint(rest[2])
                gv = _toint(rest[3])
                local_nm   = str(rest[1]).strip() if rest[1] else None
                visitante_nm = str(rest[4]).strip() if rest[4] else None

                # Layout B (becbuc audit): txt_real en rest[19], local=rest[6], vis=rest[11]
                if (gl is None or gv is None) and rest[19] is not None:
                    gl2, gv2 = _parse_score_txt(rest[19])
                    if gl2 is not None:
                        gl, gv = gl2, gv2
                        # local en rest[6], visitante en rest[11]
                        local_nm     = str(rest[6]).strip()  if rest[6]  else local_nm
                        visitante_nm = str(rest[11]).strip() if rest[11] else visitante_nm

                # Layout C (Por Partido / ranking export): P{N} en col 2; score real en rest[5]
                # rest[0]=P{N}, rest[1]=fase, rest[2]=local, rest[3]=visitante, rest[4]=pred, rest[5]=real
                if (gl is None or gv is None) and rest[5] is not None:
                    gl3, gv3 = _parse_score_txt(rest[5])
                    if gl3 is not None:
                        gl, gv = gl3, gv3
                        local_nm     = str(rest[2]).strip() if rest[2] else local_nm
                        visitante_nm = str(rest[3]).strip() if rest[3] else visitante_nm

                # Layout D (TBL MASTER "40- RESULTADOS OFICIALES"): P{N} at ci=0
                # rest[9]=EQUIPO1, rest[11]=GOLES_L, rest[13]=GOLES_V, rest[14]=EQUIPO2
                # Stats: rest[29]=amarillas, rest[30]=rojas, rest[31]=VAR, rest[33]=minuto_gol
                if (gl is None or gv is None) and ci == 0:
                    gl4 = _toint(rest[11])
                    gv4 = _toint(rest[13])
                    if gl4 is not None and gv4 is not None:
                        gl, gv = gl4, gv4
                        local_nm     = str(rest[9]).strip()  if rest[9]  else local_nm
                        visitante_nm = str(rest[14]).strip() if rest[14] else visitante_nm
                # Extract stats from TBL MASTER RESULTADOS (ci==0 row)
                if ci == 0 and num not in excel_stats:
                    _am4 = _toint(rest[29])
                    _ro4 = _toint(rest[30])
                    _vr4 = _toint(rest[31])
                    _mn4 = _toint(rest[33])
                    if any(x is not None for x in [_am4, _ro4, _vr4, _mn4]):
                        excel_stats[num] = {'am': _am4, 'ro': _ro4, 'var_': _vr4, 'min': _mn4}

                if gl is not None and gv is not None and num not in excel_partidos:
                    excel_partidos[num] = {
                        'num': num, 'gl': gl, 'gv': gv,
                        'local': local_nm,
                        'visitante': visitante_nm,
                        'hoja': sheet_name
                    }

    if not excel_partidos:
        return {"ok": False, "msg": "No se encontraron partidos (formato P{N}) en el Excel. Verificá que sea el Excel de Transparencia de BECBUC (hoja 'becbuc audit' con columna id_partido)."}

    # ── Comparar resultados de partido ────────────────────────────────────────────
    from openpyxl.styles import PatternFill, Font, Alignment, Border, Side
    from openpyxl.utils import get_column_letter

    diffs        = []   # diffs de resultado (goles)
    ok_list      = []   # partidos con goles coincidentes
    ok_nums      = set()
    score_diffs  = []   # diffs de puntaje por apostador ("becbuc audit" sheet)
    score_ok     = 0
    global_diffs = []   # diffs de puntaje_global ("Globales" sheet)
    global_ok    = 0
    _alias_to_id = {}   # alias.lower() -> apostador_id (poblado si hay sheet "becbuc audit")

    for num in sorted(excel_partidos.keys()):
        xp = excel_partidos[num]
        bp = bd_map.get(num)
        if not bp:
            diffs.append({'num': num, 'fase': '?', 'local': xp.get('local', '?'),
                          'visitante': xp.get('visitante', '?'), 'alias': '',
                          'campo': 'Partido', 'excel_val': f"{xp['gl']}-{xp['gv']}",
                          'bd_val': 'NO EXISTE EN BD', 'tipo': 'missing',
                          'diagnostico': 'partido no encontrado en BD'})
            continue
        partido_ok = True
        for campo, ex_v, bd_v in [
            ('Goles local', xp['gl'], bp.gl),
            ('Goles visit.', xp['gv'], bp.gv),
        ]:
            if ex_v != bd_v:
                diffs.append({'num': num, 'fase': bp.fase_nom or '',
                              'local': bp.local_nom, 'visitante': bp.visit_nom, 'alias': '',
                              'campo': campo, 'excel_val': ex_v, 'bd_val': bd_v, 'tipo': 'diff',
                              'diagnostico': 'resultado de partido no coincide'})
                partido_ok = False
        if partido_ok:
            ok_list.append({'num': num, 'local': bp.local_nom, 'visitante': bp.visit_nom,
                            'gl': bp.gl, 'gv': bp.gv})
            ok_nums.add(num)

    # ── Columnas de puntaje en "becbuc audit" (0-indexed): label, idx, campo BD ──
    # -- Comparar stats (amarillas/rojas/VAR/minuto) de TBL MASTER RESULTADOS ----
    stats_diffs = []
    if excel_stats:
        for _snum in sorted(excel_stats.keys()):
            _xs = excel_stats[_snum]
            _bp = bd_map.get(_snum)
            if not _bp:
                continue
            for _scampo, _xl_v, _bd_attr in [
                ('Amarillas',  _xs.get('am'),   'bd_am'),
                ('Rojas',      _xs.get('ro'),   'bd_ro'),
                ('VAR',        _xs.get('var_'), 'bd_var'),
                ('Minuto gol', _xs.get('min'),  'bd_min'),
            ]:
                if _xl_v is None:
                    continue
                _bd_v = getattr(_bp, _bd_attr, None)
                if _bd_v is None:
                    _bd_v = 0 if _bd_attr != 'bd_min' else None
                if _xl_v != _bd_v:
                    stats_diffs.append({
                        'num': _snum, 'fase': _bp.fase_nom or '',
                        'local': _bp.local_nom, 'visitante': _bp.visit_nom,
                        'campo': _scampo, 'excel_val': _xl_v, 'bd_val': _bd_v,
                    })

    _SCORE_COLS = [
        ("A- GANA-EMPATA-PIERDE", 29, "pts_resultado"),
        ("B- RESULTADO EXACTO",   30, "pts_marcador"),
        ("C- AMARILLAS",          31, "pts_amarillas"),
        ("D- ROJAS",              32, "pts_rojas"),
        ("E- VAR",                33, "pts_var"),
        ("F- PENALES",            34, "pts_penales_partido"),
        ("G- 1ER GOL",            35, "pts_minuto"),
        ("H- TANDA PENALES",      36, "pts_penales_tanda"),
    ]

    # ── Comparar puntajes por apostador (sheet "becbuc audit") ───────────────────
    if "becbuc audit" in wb_in.sheetnames:
        ws_audit = wb_in["becbuc audit"]

        _pd_r = (await db.execute(text("""
            SELECT pd.apostador_id, pd.partido_id,
                   COALESCE(pd.pts_resultado, 0)        AS pts_resultado,
                   COALESCE(pd.pts_marcador, 0)         AS pts_marcador,
                   COALESCE(pd.pts_amarillas, 0)        AS pts_amarillas,
            ...
            WHERE pd.torneo_id = :tid
        """), {"tid": torneo_id})).fetchall()
        _pd_map = {(r.apostador_id, r.partido_id): dict(r._mapping) for r in _pd_r}

        _pinfo_r = (await db.execute(text("""
            SELECT p.numero_fifa, p.id AS partido_id,
                   COALESCE(el.nombre_es, el.nombre) AS local_nom,
                   COALESCE(ev.nombre_es, ev.nombre) AS visit_nom,
                   f.nombre AS fase_nom
            FROM partido p
            ...
            WHERE f.torneo_id = :tid AND p.estado = 'finalizado'
        """), {"tid": torneo_id})).fetchall()
        _num_to_pid = {r.numero_fifa: (r.partido_id, r.local_nom, r.visit_nom, r.fase_nom)
                       for r in _pinfo_r if r.numero_fifa}

        _alias_r = (await db.execute(text("""
            SELECT DISTINCT a.apostador_id, a.nombre_apostador AS apostador
            FROM apuesta a
            JOIN partido p ON p.id = a.partido_id
            JOIN fase    f ON f.id = p.fase_id
            WHERE f.torneo_id = :tid AND a.nombre_apostador IS NOT NULL
        """), {"tid": torneo_id})).fetchall()
        _alias_to_id = {r.apostador.lower().strip(): r.apostador_id
                        for r in _alias_r if r.apostador}

        _PAT3 = _re.compile(r'^P\s*0*(\d{1,3})$', _re.IGNORECASE)
        for _row in ws_audit.iter_rows(min_row=2, values_only=True):
            if not _row or len(_row) < 2 or _row[1] is None:
                continue
            _m = _PAT3.match(str(_row[1]).strip())
            if not _m:
                continue
            _num = int(_m.group(1))
            _alias = str(_row[6]).strip() if len(_row) > 6 and _row[6] is not None else ""
            _apos_id = _alias_to_id.get(_alias.lower())
            if _apos_id is None:
                continue
            _pid_info = _num_to_pid.get(_num)
            if not _pid_info:
                continue
            _pid, _loc, _vis, _fase = _pid_info

            _xl = {field: (_toint(_row[idx]) or 0) if idx < len(_row) else 0
                   for _, idx, field in _SCORE_COLS}
            _bd_raw = _pd_map.get((_apos_id, _pid))
            _bd_vals = ({f: int(_bd_raw.get(f, 0) or 0) for _, _, f in _SCORE_COLS}
                        if _bd_raw else {f: 0 for _, _, f in _SCORE_COLS})

            _bad = [(lbl, _xl[field], _bd_vals[field])
                    for lbl, _, field in _SCORE_COLS if _xl[field] != _bd_vals[field]]
            # Comparar TOTAL (suma de items contemplados en Excel)
            _xl_total_sum = sum(_xl[field] for _, _, field in _SCORE_COLS)
            _bd_total_sum = sum(_bd_vals[field] for _, _, field in _SCORE_COLS)
            if _xl_total_sum != _bd_total_sum and not _bad:
                _bad.append(("TOTAL", _xl_total_sum, _bd_total_sum))

            if not _bad:
                score_ok += 1
                continue

            _bad_labels = [t[0] for t in _bad]
            _xl_total = sum(_xl.values())
            _bd_total = sum(_bd_vals.values())

            if _num not in ok_nums:
                _diag = f"resultado de partido no coincide → {', '.join(_bad_labels)}"
            elif _bd_raw is None:
                _diag = f"error de algoritmo becbuc — puntaje no calculado → {', '.join(_bad_labels)}"
            elif _bd_total == 0 and _xl_total > 0:
                _diag = f"error de algoritmo becbuc — puntaje en 0 → {', '.join(_bad_labels)}"
            else:
                _diag = f"error del excel → {', '.join(_bad_labels)}"

            score_diffs.append({
                'num': _num, 'fase': _fase or '', 'alias': _alias,
                'local': _loc, 'visitante': _vis,
                'campos': _bad_labels,
                'excel_vals': {t[0]: t[1] for t in _bad},
                'bd_vals':    {t[0]: t[2] for t in _bad},
                'diagnostico': _diag,
            })

    # ── Comparar puntajes globales (sheet "Globales") ────────────────────────────
    _GLOBAL_COLS = [
        ("A- CAMPEON",        "pts_campeon"),
        ("B- FINALISTAS",     "pts_finalistas"),
        ("C- GOLEADOR",       "pts_goleador"),
        ("D- PEOR EQUIPO",    "pts_peor_equipo"),
        ("E- MAYOR GOLEADA",  "pts_mayor_goleada"),
        ("F- ETAPA PARAGUAY", "pts_etapa_paraguay"),
        ("G- GOLES PARAGUAY", "pts_goles_paraguay"),
    ]

    if "Globales" in wb_in.sheetnames and _alias_to_id:
        ws_glob = wb_in["Globales"]

        _pg_r = (await db.execute(text("""
            SELECT pg.apostador_id,
                   COALESCE(pg.pts_campeon, 0)          AS pts_campeon,
                   COALESCE(pg.pts_finalistas, 0)       AS pts_finalistas,
                   COALESCE(pg.pts_goleador, 0)         AS pts_goleador,
                   COALESCE(pg.pts_peor_equipo, 0)      AS pts_peor_equipo,
                   COALESCE(pg.pts_mayor_goleada, 0)    AS pts_mayor_goleada,
                   COALESCE(pg.pts_etapa_paraguay, 0)   AS pts_etapa_paraguay,
                   COALESCE(pg.pts_goles_paraguay, 0)   AS pts_goles_paraguay
            FROM puntaje_global pg
            WHERE pg.torneo_id = :tid
        """), {"tid": torneo_id})).fetchall()
        _pg_map = {r.apostador_id: dict(r._mapping) for r in _pg_r}

        _glob_alias_col = 0
        _glob_col_idx: dict = {}
        _glob_hdr_found = False
        for _gr in ws_glob.iter_rows(values_only=True):
            if not _gr:
                continue
            if not _glob_hdr_found:
                _glob_hdr = [str(v).strip() if v else "" for v in _gr]
                for _gi, _gh in enumerate(_glob_hdr):
                    if "alias" in _gh.lower() or ("apostador" in _gh.lower() and "id" not in _gh.lower()):
                        _glob_alias_col = _gi
                    for _glbl, _gfield in _GLOBAL_COLS:
                        if _glbl.split("-")[0].strip().lower() in _gh.lower() or _glbl.lower() in _gh.lower():
                            _glob_col_idx[_gfield] = _gi
                _glob_hdr_found = True
                continue

            _gl_alias = str(_gr[_glob_alias_col]).strip() if _gr[_glob_alias_col] is not None else ""
            if not _gl_alias or _gl_alias.lower() in ("alias", "apostador", "nombre"):
                continue
            _gl_apos_id = _alias_to_id.get(_gl_alias.lower())
            if _gl_apos_id is None:
                continue

            _bd_pg = _pg_map.get(_gl_apos_id)
            _bd_pg_v = ({f: int(_bd_pg.get(f, 0) or 0) for _, f in _GLOBAL_COLS}
                        if _bd_pg else {f: 0 for _, f in _GLOBAL_COLS})
            _xl_pg = {}
            for _glbl, _gf in _GLOBAL_COLS:
                _gidx = _glob_col_idx.get(_gf)
                _xl_pg[_gf] = (_toint(_gr[_gidx]) or 0) if _gidx is not None and _gidx < len(_gr) else 0

            _bad_g = [(lbl, _xl_pg[field], _bd_pg_v[field])
                      for lbl, field in _GLOBAL_COLS if _xl_pg[field] != _bd_pg_v[field]]
            if not _bad_g:
                global_ok += 1
                continue

            _bad_g_labels = [t[0] for t in _bad_g]
            if _bd_pg is None:
                _gdiag = f"error de algoritmo becbuc — global no calculado → {', '.join(_bad_g_labels)}"
            elif sum(_bd_pg_v.values()) == 0 and sum(_xl_pg.values()) > 0:
                _gdiag = f"error de algoritmo becbuc — globales en 0 → {', '.join(_bad_g_labels)}"
            else:
                _gdiag = f"error del excel (globales) → {', '.join(_bad_g_labels)}"

            global_diffs.append({
                'alias': _gl_alias, 'campos': _bad_g_labels,
                'excel_vals': {t[0]: t[1] for t in _bad_g},
                'bd_vals':    {t[0]: t[2] for t in _bad_g},
                'diagnostico': _gdiag,
            })

    # ── Early return JSON si se solicitó ─────────────────────────────────────────
    if return_json:
        return {
            "ok": True,
            "total_excel": len(excel_partidos),
    ...
    # ── Generar Excel de diferencias ─────────────────────────────────────────────
    wb_out = _opx.Workbook()
    ws_d = wb_out.active
    ws_d.title = "Diferencias"
    ...
    M_FILL  = PatternFill("solid", fgColor="FEF9C3")
    W_FILL  = PatternFill("solid", fgColor="FFF7ED")

    def _c(ws, r, c, v, fill=None, bold=False, color=None, align='left'):
        cell = ws.cell(row=r, column=c, value=v)
        if fill: cell.fill = fill
        cell.font = Font(bold=bold, color=color or '1E293B', size=10)
        cell.alignment = Alignment(horizontal=align, vertical='center', wrap_text=True)
        cell.border = thin
        return cell

    # ── Hoja "Diferencias" (resultados / goles) ──────────────────────────────────
    ws_d.merge_cells('A1:I1')
    _c(ws_d, 1, 1, f'Auditoría BECBUC — Excel vs BD — {_dt.date.today()}', fill=H_FILL, bold=True, color='E2E8F0', align='center')
    ws_d.row_dimensions[1].height = 22
    _c(ws_d, 2, 1, f'En Excel: {len(excel_partidos)}', bold=False)
    _c(ws_d, 2, 3, f'OK resultados: {len(ok_list)}', color='166534', bold=True)
    _c(ws_d, 2, 6, f'Dif. resultados: {len(diffs)}', color='B91C1C', bold=True)
    _c(ws_d, 2, 8, f'Dif. puntajes: {len(score_diffs)}', color='92400E', bold=True)
    if stats_diffs:
        _c(ws_d, 2, 9, f'Dif. stats: {len(stats_diffs)}', color='5B21B6', bold=True)

    _HDR9 = ['P#', 'Fase', 'Local', 'Visitante', 'ALIAS', 'Campo', 'Excel', 'BD', 'DIAGNÓSTICO']
    for ci, h in enumerate(_HDR9, 1):
        _c(ws_d, 4, ci, h, fill=H_FILL, bold=True, color='E2E8F0', align='center')
    ws_d.row_dimensions[4].height = 18

    if not diffs:
        ws_d.merge_cells('A5:I5')
        _c(ws_d, 5, 1, '✅ Sin diferencias de resultado — los datos del Excel coinciden con la BD', color='166534', bold=True)
    else:
        for ri, d in enumerate(diffs, 5):
            fl = M_FILL if d['tipo'] == 'missing' else D_FILL
            _c(ws_d, ri, 1, f"P{d['num']}", fill=fl, bold=True, align='center')
            _c(ws_d, ri, 2, d.get('fase', ''), fill=fl)
            _c(ws_d, ri, 3, d.get('local', ''), fill=fl)
            _c(ws_d, ri, 4, d.get('visitante', ''), fill=fl)
            _c(ws_d, ri, 5, d.get('alias', ''), fill=fl)
            _c(ws_d, ri, 6, d.get('campo', ''), fill=fl, bold=True)
            _c(ws_d, ri, 7, str(d.get('excel_val', '')), fill=fl, color='B91C1C', bold=True, align='center')
            _c(ws_d, ri, 8, str(d.get('bd_val', '')), fill=fl, color='166534', bold=True, align='center')
            _c(ws_d, ri, 9, d.get('diagnostico', ''), fill=fl, color='92400E')
            ws_d.row_dimensions[ri].height = 20

    for ci, w in enumerate([7, 20, 24, 24, 16, 16, 10, 10, 52], 1):
        ws_d.column_dimensions[get_column_letter(ci)].width = w

    # ── Hoja "Sin Diferencias" ───────────────────────────────────────────────────
    # -- Hoja "Stats Diff" (amarillas/rojas/VAR/minuto desde TBL MASTER) ----------
    if stats_diffs:
        _S_FILL = PatternFill("solid", fgColor="EDE9FE")
        ws_st = wb_out.create_sheet("Stats Diff")
        for _ci2, _h2 in enumerate(['P#', 'Fase', 'Local', 'Visitante', 'Campo', 'Excel', 'BD', 'Diff'], 1):
            _c(ws_st, 1, _ci2, _h2, fill=H_FILL, bold=True, color='E2E8F0', align='center')
        _c(ws_st, 2, 1, f'Diferencias stats: {len(stats_diffs)}', color='5B21B6', bold=True)
        for _ri2, _d2 in enumerate(stats_diffs, 3):
            _c(ws_st, _ri2, 1, f"P{_d2['num']}", fill=_S_FILL, bold=True, align='center')
            _c(ws_st, _ri2, 2, _d2.get('fase', ''), fill=_S_FILL)
            _c(ws_st, _ri2, 3, _d2.get('local', ''), fill=_S_FILL)
            _c(ws_st, _ri2, 4, _d2.get('visitante', ''), fill=_S_FILL)
            _c(ws_st, _ri2, 5, _d2.get('campo', ''), fill=_S_FILL, bold=True)
            _c(ws_st, _ri2, 6, str(_d2.get('excel_val', '')), fill=_S_FILL, color='B91C1C', bold=True, align='center')
            _c(ws_st, _ri2, 7, str(_d2.get('bd_val', '')), fill=_S_FILL, color='166534', bold=True, align='center')
            _dv2 = (_d2.get('excel_val') or 0) - (_d2.get('bd_val') or 0)
            _c(ws_st, _ri2, 8, f"{_dv2:+d}", fill=_S_FILL, color='5B21B6', bold=True, align='center')
            ws_st.row_dimensions[_ri2].height = 18
        for _ci2, _w2 in enumerate([7, 20, 24, 24, 14, 10, 10, 8], 1):
            ws_st.column_dimensions[get_column_letter(_ci2)].width = _w2

    ws_ok = wb_out.create_sheet("Sin Diferencias")
    for ci, h in enumerate(['P#', 'Local', 'Marcador Excel', 'Marcador BD'], 1):
        _c(ws_ok, 1, ci, h, fill=H_FILL, bold=True, color='E2E8F0', align='center')
    for ri, p in enumerate(ok_list, 2):
        _c(ws_ok, ri, 1, f"P{p['num']}", fill=OK_FILL, bold=True, align='center')
        _c(ws_ok, ri, 2, p['local'], fill=OK_FILL)
        _c(ws_ok, ri, 3, f"{excel_partidos[p['num']]['gl']}-{excel_partidos[p['num']]['gv']}", fill=OK_FILL, align='center')
        _c(ws_ok, ri, 4, f"{p['gl']}-{p['gv']}", fill=OK_FILL, align='center')
    for ci, w in enumerate([7, 26, 16, 16], 1):
        ws_ok.column_dimensions[get_column_letter(ci)].width = w

    # ── Hoja "Puntajes" (diffs por apostador × partido) ─────────────────────────
    if score_diffs or score_ok > 0:
        ws_sc = wb_out.create_sheet("Puntajes")
        _SC_HDR = ['P#', 'Fase', 'Local', 'Visitante', 'ALIAS', 'Campos con diferencia', 'Vals Excel', 'Vals BD', 'DIAGNÓSTICO']
        for ci, h in enumerate(_SC_HDR, 1):
            _c(ws_sc, 1, ci, h, fill=H_FILL, bold=True, color='E2E8F0', align='center')
        ws_sc.row_dimensions[1].height = 18
        _c(ws_sc, 2, 1, f'OK: {score_ok}  |  Con diferencias: {len(score_diffs)}', color='166534', bold=True)
        if not score_diffs:
            ws_sc.merge_cells('A3:I3')
            _c(ws_sc, 3, 1, '✅ Todos los puntajes de apostadores coinciden con la BD', color='166534', bold=True)
        else:
            for ri, d in enumerate(score_diffs, 3):
                _xl_str = '  |  '.join(f"{k}: {v}" for k, v in d.get('excel_vals', {}).items())
                _bd_str = '  |  '.join(f"{k}: {v}" for k, v in d.get('bd_vals', {}).items())
                _c(ws_sc, ri, 1, f"P{d['num']}", fill=W_FILL, bold=True, align='center')
                _c(ws_sc, ri, 2, d.get('fase', ''), fill=W_FILL)
                _c(ws_sc, ri, 3, d.get('local', ''), fill=W_FILL)
                _c(ws_sc, ri, 4, d.get('visitante', ''), fill=W_FILL)
                _c(ws_sc, ri, 5, d.get('alias', ''), fill=W_FILL, bold=True)
                _c(ws_sc, ri, 6, ', '.join(d.get('campos', [])), fill=W_FILL, color='B91C1C')
                _c(ws_sc, ri, 7, _xl_str, fill=W_FILL, color='B91C1C', align='center')
                _c(ws_sc, ri, 8, _bd_str, fill=W_FILL, color='166534', align='center')
                _c(ws_sc, ri, 9, d.get('diagnostico', ''), fill=W_FILL, color='92400E')
                ws_sc.row_dimensions[ri].height = 28
        for ci, w in enumerate([7, 18, 22, 22, 16, 42, 36, 36, 55], 1):
            ws_sc.column_dimensions[get_column_letter(ci)].width = w

    # ── Hoja "Globales" (diffs puntaje_global) ───────────────────────────────────
    if global_diffs or global_ok > 0:
        ws_gld = wb_out.create_sheet("Globales")
        _GL_HDR = ['ALIAS', 'Items con diferencia', 'Vals Excel', 'Vals BD', 'DIAGNÓSTICO']
        for ci, h in enumerate(_GL_HDR, 1):
            _c(ws_gld, 1, ci, h, fill=H_FILL, bold=True, color='E2E8F0', align='center')
        ws_gld.row_dimensions[1].height = 18
        _c(ws_gld, 2, 1, f'OK: {global_ok}  |  Con diferencias: {len(global_diffs)}', color='166534', bold=True)
        if not global_diffs:
            ws_gld.merge_cells('A3:E3')
            _c(ws_gld, 3, 1, '✅ Todos los puntajes globales coinciden con la BD', color='166534', bold=True)
        else:
            for ri, d in enumerate(global_diffs, 3):
                _xl_str = '  |  '.join(f"{k}: {v}" for k, v in d.get('excel_vals', {}).items())
                _bd_str = '  |  '.join(f"{k}: {v}" for k, v in d.get('bd_vals', {}).items())
                _c(ws_gld, ri, 1, d.get('alias', ''), fill=W_FILL, bold=True)
                _c(ws_gld, ri, 2, ', '.join(d.get('campos', [])), fill=W_FILL, color='B91C1C')
                _c(ws_gld, ri, 3, _xl_str, fill=W_FILL, color='B91C1C', align='center')
                _c(ws_gld, ri, 4, _bd_str, fill=W_FILL, color='166534', align='center')
                _c(ws_gld, ri, 5, d.get('diagnostico', ''), fill=W_FILL, color='92400E')
                ws_gld.row_dimensions[ri].height = 28
        for ci, w in enumerate([18, 42, 36, 36, 55], 1):
            ws_gld.column_dimensions[get_column_letter(ci)].width = w
    ...
    return _SR(buf, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
               headers={"Content-Disposition": f"attachment; filename=becbuc_auditoria_diff_{ts}.xlsx"})


# ── Comparacion items BD vs API raw para fase activa ────────────────────────

@router.get("/admin/auditoria-items-activos/{torneo_id}", summary="Comparacion puntaje_detalle vs datos raw de partido para fase activa (admin)")
async def auditoria_items_activos(
    torneo_id: int,
    current: CurrentAdmin,
    db: DBSession,
) -> dict:
    """
    Para cada apostador: totales H,I,J,K,L,M,N,O en la fase activa (no bloqueada).
    Tambien retorna, por partido de la fase, los datos raw de partido (goles, amarillas, etc.)
    para comparar si el scoring es correcto (BD) vs lo que sync extrajo de API/ESPN.
    Excluye: pts_equipo (clasificados) y globales A-G.
    """
    _check_admin(current)

    # 1. Fase activa (no bloqueada, con partidos finalizados)
    r_fases = await db.execute(text("""
        SELECT f.id, f.nombre, f.tipo,
               COUNT(p.id) FILTER (WHERE p.estado='finalizado') AS finalizados,
               COUNT(p.id) AS total
        FROM fase f
        JOIN partido p ON p.fase_id = f.id
        WHERE f.torneo_id = :tid
          AND NOT COALESCE(f.bloqueada, FALSE)
        GROUP BY f.id, f.nombre, f.tipo
        HAVING COUNT(p.id) FILTER (WHERE p.estado='finalizado') > 0
        ORDER BY f.id DESC
        LIMIT 1
    """), {"tid": torneo_id})
    fase_row = r_fases.mappings().first()
    if not fase_row:
        return {"ok": False, "message": "No hay fase activa con partidos finalizados"}

    fase_id   = fase_row["id"]
    fase_nom  = fase_row["nombre"]
    fase_tipo = fase_row["tipo"]

    # 2. Partidos finalizados de la fase con datos raw
    r_part = await db.execute(text("""
        SELECT p.id, p.numero_fifa,
               ...
               COALESCE(p.penales_partido, 0) AS penales_partido,
               p.minuto_primer_gol,
               p.penales_local   AS pen_tanda_local,
               p.penales_visitante AS pen_tanda_visitante,
               COALESCE(p.rojas, 0) AS rojas,
               p.fecha
        FROM partido p
        JOIN equipo el ON el.id = p.equipo_local_id
        ...
        ORDER BY p.numero_fifa
    """), {"fid": fase_id})
    partidos = [dict(row) for row in r_part.mappings().all()]
    partido_ids = [p["id"] for p in partidos]

    if not partido_ids:
        return {"ok": False, "message": "No hay partidos finalizados en la fase activa"}

    ids_sql = ",".join(str(i) for i in partido_ids)

    # 3. Puntaje_detalle por apostador x partido para la fase
    r_det = await db.execute(text(f"""
        SELECT pd.apostador_id, pd.partido_id,
               COALESCE(pd.pts_resultado, 0)        AS "H",
               COALESCE(pd.pts_marcador, 0)         AS "I",
        ...
        WHERE pd.partido_id IN ({ids_sql})
    """))
    det_rows = r_det.mappings().all()

    # Obtener usernames desde app_db
    async with _app_engine.connect() as app_conn:
        r_users = await app_conn.execute(text("SELECT id, username FROM users"))
        uid_map = {r["id"]: r["username"] for r in r_users.mappings().all()}

    # 4a. Predicciones de apostadores para re-scoring por fuente
    r_preds = await db.execute(text(f"""
        SELECT apostador_id, partido_id,
               COALESCE(pred_amarillas, 0)        AS pred_J,
               COALESCE(pred_rojas, 0)            AS pred_K,
               COALESCE(pred_var, 0)              AS pred_L,
               COALESCE(pred_penales_partido, 0)  AS pred_M,
               pred_minuto_gol                    AS pred_N
        FROM apuesta
        WHERE partido_id IN ({ids_sql})
    """))
    preds_map = {}   # (aid, pid) -> dict
    for pr in r_preds.mappings().all():
        preds_map[(pr["apostador_id"], pr["partido_id"])] = dict(pr)

    # 4b. partido_stats_fuentes (api_* y espn_*) — puede no existir para todos los partidos
    stats_fuentes: dict[int, dict] = {}  # partido_id -> fuentes
    try:
        r_sf = await db.execute(text(f"""
            SELECT partido_id,
                   api_amarillas, api_rojas, api_decisiones_var,
                   api_penales_partido, api_minuto_primer_gol,
                   espn_amarillas, espn_rojas, espn_decisiones_var,
                   espn_penales_partido, espn_minuto_primer_gol
            FROM partido_stats_fuentes
            WHERE partido_id IN ({ids_sql})
        """))
        for sf in r_sf.mappings().all():
            stats_fuentes[sf["partido_id"]] = dict(sf)
    except Exception:
        pass  # tabla puede no existir o no tener datos — inferencia basada solo en partido.*

    # Helper: re-score items J/K/L/M para una fuente dada
    def _rescore_jklm(pred: dict, raw_j, raw_k, raw_l, raw_m) -> dict:
        return {
            "J": 1 if pred.get("pred_J") == (raw_j or 0) else 0,
            "K": 1 if pred.get("pred_K") == (raw_k or 0) else 0,
            "L": 1 if pred.get("pred_L") == (raw_l or 0) else 0,
            "M": 1 if pred.get("pred_M") == (raw_m or 0) else 0,
        }

    # Helper: minuto gol — 1pt al mas cercano (entre todos los apostadores para ese partido)
    def _minuto_pts(aid, pid, all_preds, real_min) -> int:
        if real_min is None:
            return 0
        dists = {}
        for (a, p), pr in all_preds.items():
            if p == pid and pr.get("pred_N") is not None:
                dists[a] = abs(pr["pred_N"] - real_min)
        if not dists:
            return 0
        min_dist = min(dists.values())
        my_pred = all_preds.get((aid, pid), {}).get("pred_N")
        if my_pred is None:
            return 0
        return 1 if abs(my_pred - real_min) == min_dist else 0

    # 4c. Agrupar: totales por apostador (BD)
    from collections import defaultdict
    totals  = defaultdict(lambda: {"H":0,"I":0,"J":0,"K":0,"L":0,"M":0,"N":0,"O":0})
    by_part = defaultdict(lambda: defaultdict(lambda: {"H":0,"I":0,"J":0,"K":0,"L":0,"M":0,"N":0,"O":0}))
    ...
    for row in det_rows:
        ...
        pid = row["partido_id"]
        for it in ITEMS:
            totals[aid][it]       += row[it]
            by_part[aid][pid][it] += row[it]

    # 4d. Source scores por apostador: {api, espn, bd}
    # Para items H, I, O: son iguales en todas las fuentes (dependen de goles/tanda)
    # Para J, K, L, M, N: re-score con api_* y espn_*
    part_map = {p["id"]: p for p in partidos}
    source_scores: dict[int, dict] = {}  # aid -> {api, espn, bd}

    all_aid_set = set(totals.keys()) | {k[0] for k in preds_map.keys()}
    for aid in all_aid_set:
        bd_tot = sum(totals[aid].values())
        api_jklm_tot  = 0
        espn_jklm_tot = 0
        bd_jklm_tot   = 0

        for pid, p in part_map.items():
            pred = preds_map.get((aid, pid), {})
            sf   = stats_fuentes.get(pid, {})
            bd_p = by_part[aid].get(pid, {})

            # BD actual J/K/L/M from puntaje_detalle
            bd_jklm_tot += bd_p.get("J",0) + bd_p.get("K",0) + bd_p.get("L",0) + bd_p.get("M",0)

            # API source: usa api_* si disponible, sino partido.* (que es el mismo sync)
            api_j = sf.get("api_amarillas",  p["amarillas"])
            api_k = sf.get("api_rojas",      p["rojas"])
            api_l = sf.get("api_decisiones_var", p["decisiones_var"])
            api_m = sf.get("api_penales_partido", p["penales_partido"])
            api_n = sf.get("api_minuto_primer_gol", p["minuto_primer_gol"])
            sc_api = _rescore_jklm(pred, api_j, api_k, api_l, api_m)
            sc_api["N"] = _minuto_pts(aid, pid, preds_map, api_n)
            api_jklm_tot += sum(sc_api.values())

            # ESPN source
            espn_j = sf.get("espn_amarillas",  p["amarillas"])
            espn_k = sf.get("espn_rojas",      p["rojas"])
            espn_l = sf.get("espn_decisiones_var", p["decisiones_var"])
            espn_m = sf.get("espn_penales_partido", p["penales_partido"])
            espn_n = sf.get("espn_minuto_primer_gol", p["minuto_primer_gol"])
            sc_espn = _rescore_jklm(pred, espn_j, espn_k, espn_l, espn_m)
            sc_espn["N"] = _minuto_pts(aid, pid, preds_map, espn_n)
            espn_jklm_tot += sum(sc_espn.values())

        # H, I, O are source-independent — take from BD
        hi_o = bd_tot - bd_jklm_tot
        source_scores[aid] = {
            "bd":   bd_tot,
            "api":  hi_o + api_jklm_tot,
            "espn": hi_o + espn_jklm_tot,
            "has_fuentes": bool(stats_fuentes),
        }

    # Construir respuesta de apostadores
    apostadores = []
    for aid, tot in sorted(totals.items(), key=lambda x: -sum(x[1].values())):
        uname = uid_map.get(aid, f"uid{aid}")
        partidos_det = []
        for p in partidos:
            pid = p["id"]
            pts = by_part[aid].get(pid, {it:0 for it in ITEMS})
            partidos_det.append({"numero_fifa": p["numero_fifa"],
                                  "local": p["local"], "visitante": p["visitante"], **pts})
        src = source_scores.get(aid, {"bd": sum(tot.values()), "api": None, "espn": None, "has_fuentes": False})
        apostadores.append({
            "apostador_id": aid,
            "username": uname,
            **tot,
    ...
    # Determinar que fuentes difieren entre si (para el informe)
    fuentes_summary: dict = {"has_fuentes": bool(stats_fuentes)}
    if stats_fuentes and apostadores:
        diffs_api  = sum(1 for a in apostadores if a["fuentes"].get("api")  != a["fuentes"].get("bd"))
        diffs_espn = sum(1 for a in apostadores if a["fuentes"].get("espn") != a["fuentes"].get("bd"))
        fuentes_summary["apostadores_dif_api"]  = diffs_api
        fuentes_summary["apostadores_dif_espn"] = diffs_espn
        fuentes_summary["nota"] = (
            f"{diffs_api} apostadores difieren API vs BD; "
            f"{diffs_espn} difieren ESPN vs BD. "
            "Comparar columnas Fuentes con el TBL CHECK para identificar la fuente correcta."
        )
    else:
        fuentes_summary["nota"] = "Sin datos en partido_stats_fuentes: fuentes API/ESPN no disponibles para esta fase. Los valores mostrados son los del partido actual (BD sync)."
    ...
    return {
        ...
        "fase_tipo": fase_tipo,
        "partidos_count": len(partidos),
        "apostadores_count": len(apostadores),
        "fuentes_summary": fuentes_summary,
        "apostadores": apostadores,
        "partidos_raw": [
            {
                "id": p["id"],
                ...
                "rojas": p["rojas"],
                "decisiones_var": p["decisiones_var"],
                "penales_partido": p["penales_partido"],
                "minuto_primer_gol": p["minuto_primer_gol"],
                "pen_tanda_local": p["pen_tanda_local"],
                "pen_tanda_visitante": p["pen_tanda_visitante"],
                "fuentes": stats_fuentes.get(p["id"], {}),
                # Flags de diff API vs BD por item
                "diffs_api": {
                    k: True
                    for k, bd_k, api_k in [
                        ("J", p["amarillas"],     stats_fuentes.get(p["id"], {}).get("api_amarillas")),
                        ("K", p["rojas"],         stats_fuentes.get(p["id"], {}).get("api_rojas")),
                        ("L", p["decisiones_var"],stats_fuentes.get(p["id"], {}).get("api_decisiones_var")),
                        ("M", p["penales_partido"],stats_fuentes.get(p["id"], {}).get("api_penales_partido")),
                        ("N", p["minuto_primer_gol"],stats_fuentes.get(p["id"], {}).get("api_minuto_primer_gol")),
                    ]
                    if api_k is not None and api_k != bd_k
                },
            }
            for p in partidos
        ],
...
# ── Comparacion activos: exportar Excel ──────────────────────────────────────

@router.get("/admin/auditoria-items-activos-excel/{torneo_id}",
            summary="Exporta Excel con ítems H-O de la fase activa por apostador (admin)")
async def auditoria_items_activos_excel(
    torneo_id: int,
    current: CurrentAdmin,
    db: DBSession,
):
    """Genera Excel con puntaje_detalle H-O para la fase activa + datos raw partidos.
    Diseñado para comparar contra TBL CHECK externo."""
    import io
    from openpyxl import Workbook
    ...
    _check_admin(current)
    # Reusar la logica del endpoint anterior
    data = await auditoria_items_activos(torneo_id, current, db)
    if not data.get("ok"):
        return {"ok": False, "message": data.get("message", "Sin datos")}

    wb  = Workbook()
    ws1 = wb.active
    ws1.title = "Ítems fase activa"
    hdr_fill = PatternFill("solid", fgColor="1E293B")
    hdr_font = Font(color="F8FAFC", bold=True)
    ITEMS = ["H","I","J","K","L","M","N","O"]

    # Encabezado
    cols = ["Apostador"] + ITEMS + ["Total BD", "Total API", "Total ESPN"]
    ...
    # Datos
    for ri, ap in enumerate(data["apostadores"], 2):
        ws1.cell(ri, 1, ap["username"])
        for ci, it in enumerate(ITEMS, 2):
            ws1.cell(ri, ci, ap.get(it, 0))
        ws1.cell(ri, len(ITEMS)+2, ap.get("total", 0))
        fuentes = ap.get("fuentes", {})
        ws1.cell(ri, len(ITEMS)+3, fuentes.get("api") or "—")
        ws1.cell(ri, len(ITEMS)+4, fuentes.get("espn") or "—")

    # Hoja 2: partidos raw
    ws2 = wb.create_sheet("Partidos raw")
    raw_cols = ["#","Local","GL","GV","Visitante","🟨","🟥","VAR","Pen⚽","⏱min","TandaL","TandaV"]
    for ci, h in enumerate(raw_cols, 1):
        c = ws2.cell(1, ci, h); c.fill = hdr_fill; c.font = hdr_font
    for ri, p in enumerate(data["partidos_raw"], 2):
        ws2.cell(ri, 1, f"P{p.get('numero_fifa','?')}")
        ws2.cell(ri, 2, p.get("local",""))
        ws2.cell(ri, 3, p.get("goles_local", ""))
        ws2.cell(ri, 4, p.get("goles_visitante", ""))
        ws2.cell(ri, 5, p.get("visitante",""))
        ws2.cell(ri, 6, p.get("amarillas",""))
        ws2.cell(ri, 7, p.get("rojas",""))
        ws2.cell(ri, 8, p.get("decisiones_var",""))
        ws2.cell(ri, 9, p.get("penales_partido",""))
        ws2.cell(ri, 10, p.get("minuto_primer_gol",""))
        ws2.cell(ri, 11, p.get("pen_tanda_local",""))
        ws2.cell(ri, 12, p.get("pen_tanda_visitante",""))

    # Hoja 3: nota fuentes
    if data.get("fuentes_summary"):
        ws3 = wb.create_sheet("Fuentes")
        ws3.cell(1, 1, "Nota")
        ws3.cell(2, 1, data["fuentes_summary"].get("nota",""))
        ws3.cell(3, 1, "Difs API vs BD")
        ws3.cell(3, 2, data["fuentes_summary"].get("apostadores_dif_api","—"))
        ws3.cell(4, 1, "Difs ESPN vs BD")
        ws3.cell(4, 2, data["fuentes_summary"].get("apostadores_dif_espn","—"))

    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    from datetime import datetime as _dtm
    ts = _dtm.now().strftime("%Y%m%d_%H%M")
    fname = f"becbuc_items_activos_{torneo_id}_{ts}.xlsx"
    return StreamingResponse(
        buf,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'attachment; filename="{fname}"'},
...
# ── Sync automático: actualizar BD desde API para partidos con diffs ─────────

@router.post("/admin/sync-diff-partidos/{torneo_id}",
             summary="Actualiza BD con datos API donde difieren para la fase activa (admin)")
async def sync_diff_partidos(
    torneo_id: int,
    current: CurrentAdmin,
    db: DBSession,
) -> dict:
    """
    Para cada partido finalizado de la fase activa que NO esté blindado:
      1. Compara valores BD vs API en partido_stats_fuentes (J/K/L/M/N).
      2. Donde API difiere de BD → UPDATE partido con valores API.
      3. Recalcula puntajes.
    Retorna lista de partidos actualizados con detalle de diffs.
    Sin datos en partido_stats_fuentes → reporta partidos sin fuente.
    """
    _check_admin(current)

    # Fase activa no bloqueada
    r_fases = await db.execute(text("""
        SELECT f.id FROM fase f
        WHERE f.torneo_id = :tid AND NOT COALESCE(f.bloqueada, FALSE)
          AND EXISTS (SELECT 1 FROM partido p WHERE p.fase_id=f.id AND p.estado='finalizado')
    ...
    # Partidos finalizados sin blindaje
    r_part = await db.execute(text("""
        SELECT p.id, p.numero_fifa,
               el.nombre AS local, ev.nombre AS visitante,
               COALESCE(p.amarillas, 0)       AS bd_j,
               COALESCE(p.rojas, 0)           AS bd_k,
               COALESCE(p.decisiones_var, 0)  AS bd_l,
               COALESCE(p.penales_partido, 0) AS bd_m,
               p.minuto_primer_gol            AS bd_n
        FROM partido p
        JOIN equipo el ON el.id = p.equipo_local_id
        JOIN equipo ev ON ev.id = p.equipo_visitante_id
        WHERE p.fase_id = :fid AND p.estado = 'finalizado'
          AND NOT COALESCE(p.datos_confirmados, FALSE)
        ORDER BY p.numero_fifa
    """), {"fid": fase_id})
    partidos = [dict(r) for r in r_part.mappings().all()]

    if not partidos:
        return {
            "ok": True,
            "message": "No hay partidos candidatos (todos blindados o no finalizados)",
            "sincronizados": [], "sin_fuentes": [],
        }

    ids_sql = ",".join(str(p["id"]) for p in partidos)
    ...
    try:
        r_sf = await db.execute(text(f"""
            SELECT partido_id,
                   api_amarillas, api_rojas, api_decisiones_var,
                   api_penales_partido, api_minuto_primer_gol
            FROM partido_stats_fuentes WHERE partido_id IN ({ids_sql})
        """))
        for row in r_sf.mappings().all():
            stats_map[row["partido_id"]] = dict(row)
    except Exception:
        pass
    ...
    for p in partidos:
        pid = p["id"]
        sf  = stats_map.get(pid)
        if not sf:
            sin_fuentes.append(f"P{p['numero_fifa']}")
            continue

        # Detectar diffs por item
        diffs = {}
        updates = {}
        checks = [
            ("amarillas",        "api_amarillas",       p["bd_j"], "J"),
            ("rojas",            "api_rojas",           p["bd_k"], "K"),
            ("decisiones_var",   "api_decisiones_var",  p["bd_l"], "L"),
            ("penales_partido",  "api_penales_partido", p["bd_m"], "M"),
            ("minuto_primer_gol","api_minuto_primer_gol",p["bd_n"],"N"),
        ]
        for col, api_key, bd_val, item in checks:
            api_val = sf.get(api_key)
            if api_val is not None and api_val != bd_val:
                updates[col] = api_val
                diffs[item]  = {"bd": bd_val, "api": api_val}
    ...
    if sincronizados:
        ...
        # Recalcular puntajes
        try:
            from app.services.scoring.registry import get_engine as _sreg
            from app.services.scoring.calculator import ScoringCalculator as _SCalc
            r_comp = await db.execute(text("""
                SELECT c.codigo FROM competicion c
                JOIN torneo t ON t.competicion_id=c.id WHERE t.id=:tid
            """), {"tid": torneo_id})
            comp_row = r_comp.mappings().first()
            engine   = _sreg(comp_row["codigo"] if comp_row else None)
            calc     = _SCalc(db)
            await calc.calculate(torneo_id, engine)
            await calc.calculate_global(torneo_id, engine)
            puntajes_ok = True
        ...
        puntajes_ok = None  # no hubo cambios

    return {
        "ok":               True,
        "sincronizados":    sincronizados,
        "sin_fuentes":      sin_fuentes,
        "total_actualizados": len(sincronizados),
        "puntajes_ok":      puntajes_ok,
        "message": (
            f"{len(sincronizados)} partido(s) actualizados con datos API. "
            f"{len(sin_fuentes)} sin datos en partido_stats_fuentes."
            + (" Puntajes recalculados." if puntajes_ok else "")
        ),
    }


# ── Sync resultados desde Excel a BD ─────────────────────────────────────────

@router.post("/admin/sync-excel-resultados/{torneo_id}")
async def sync_excel_resultados(
    torneo_id: int,
    file: UploadFile = File(...),
    current: CurrentAdmin = None,
    db: DBSession = None,
):
    """Copia goles del Excel a la tabla partido y recalcula puntajes de la fase vigente."""
    import io, openpyxl as _opx_s, re as _re_s
    content = await file.read()
    try:
        wb = _opx_s.load_workbook(io.BytesIO(content), data_only=True)
    except Exception as e:
        raise HTTPException(400, f"No se pudo leer el Excel: {e}")

    PAT_S = _re_s.compile(r'^P\s*0*(\d{1,3})$', _re_s.IGNORECASE)
    PAT_SC = _re_s.compile(r'^(\d+)\s*[–\-]\s*(\d+)$')
    ...
    def _ps(v):
        ...
        m2 = PAT_SC.match(s)
        if m2: return int(m2.group(1)), int(m2.group(2))
        m3 = _re_s.search(r'(\d+)\s+vs\s+(\d+)', s, _re_s.IGNORECASE)
        if m3: return int(m3.group(1)), int(m3.group(2))
        return None, None
    ...
    for sn in wb.sheetnames:
        ...
        for row in ws.iter_rows(values_only=True):
            for ci, cell in enumerate(row):
                if cell is None: continue
                m = PAT_S.match(str(cell).strip())
                if not m: continue
                num = int(m.group(1))
                if not (1 <= num <= 104): continue
                rest = list(row[ci:ci+40]) + [None]*40
                gl = _ti(rest[2]); gv = _ti(rest[3])
                if (gl is None or gv is None) and rest[19] is not None:
                    gl2, gv2 = _ps(rest[19])
                    if gl2 is not None: gl, gv = gl2, gv2
                if (gl is None or gv is None) and ci == 0:
                    gl4 = _ti(rest[11]); gv4 = _ti(rest[13])
                    if gl4 is not None and gv4 is not None: gl, gv = gl4, gv4
                if gl is not None and gv is not None and num not in excel_g:
                    excel_g[num] = (gl, gv)

    if not excel_g:
        raise HTTPException(400, "No se encontraron partidos (P{N}) en el Excel.")

    actualizados = []
    for num, (gl, gv) in excel_g.items():
        res = await db.execute(text("""
            UPDATE partido SET goles_local=:gl, goles_visitante=:gv,
                datos_confirmados=TRUE,
                estado=CASE WHEN estado='programado' OR estado='pendiente' THEN 'finalizado'
                            ELSE estado END
            WHERE torneo_id=:tid AND numero_fifa=:num
            RETURNING id, numero_fifa
        """), {"gl": gl, "gv": gv, "tid": torneo_id, "num": num})
        row = res.fetchone()
        if row:
            actualizados.append({"num": num, "gl": gl, "gv": gv})
    await db.commit()

    bracket_ok = False
    try:
        await _avanzar_bracket(db, torneo_id)
        bracket_ok = True
    except Exception:
        pass
    ...
    try:
        from app.services.scoring.calculator import ScoringCalculator
        from app.services.scoring.registry import get_engine
        cod_r = await db.execute(text(
            "SELECT c.codigo FROM torneo t JOIN competicion c ON c.id=t.competicion_id WHERE t.id=:tid"),
            {"tid": torneo_id})
        cod_row = cod_r.fetchone()
        codigo = (cod_row[0] or "default") if cod_row else "default"
        engine = get_engine(codigo)
        puntajes = await ScoringCalculator(db).calculate(torneo_id, engine)
        try:
            await ScoringCalculator(db).calculate_global(torneo_id, engine)
        except Exception:
