...
# ---------------------------------------------------------------------------
# Endpoint temporal: registrar BECBUC-SyncAPI en Task Scheduler
# ---------------------------------------------------------------------------
...
async def register_sync_task(_admin: CurrentAdmin):
    ...
    # 1. Eliminar tarea anterior si existe
    r1 = subprocess.run(
    ...
    # 2. Crear nueva tarea (corre como usuario actual, no necesita UAC)
    r2 = subprocess.run(
        [
            "schtasks", "/create",
            "/tn", "BECBUC-SyncAPI",
    ...
    # 3. Disparar ejecucion de prueba si la creacion fue exitosa
    if r2.returncode == 0:
        r3 = subprocess.run(
